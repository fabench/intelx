...
function setPartyProfile(party){
  selParty=party;
  simDetailParty=party; // keep sim details in sync with selected party
  selDist=null;
  activeTopTab='party';
...
// Track which party is selected for status colouring in sim details
let simDetailParty = 'RNI';

function getSimDistStatus(distName, d, party) {
  // Status relative to the selected party:
  // Does the selected party win a seat here? What's its margin?
  const allWinners = rankBySeats(d.v, d.s);
  const partyWins = allWinners.includes(party);
  const tot = Object.values(d.v).reduce((a,b)=>a+b,0)||1;
  const sorted = Object.entries(d.v).sort((a,b)=>b[1]-a[1]);
  const partyRank = sorted.findIndex(([p])=>p===party)+1;
  const partyVotes = d.v[party]||0;
  const partyPct = partyVotes/tot*100;

  if(!partyWins){
    // Not winning — how close?
  ...
  // Winning — how safe?
  const seatsWon = allWinners.filter(w=>w===party).length;
  // Margin: gap between last party seat and first loser
  const partyPositions = allWinners.map((w,i)=>({w,i})).filter(x=>x.w===party);
  const lastPartyPos = partyPositions[partyPositions.length-1]?.i ?? (d.s-1);
  const firstLoser = sorted[d.s];
  const lastPartyVotes = sorted[lastPartyPos]?.[1]||partyVotes;
  const margin = firstLoser ? (lastPartyVotes - firstLoser[1])/tot*100 : 20;
  if(margin<3) return 'tossup';
  if(margin<8) return 'battleground';
  if(margin<15) return 'lean';
  return 'safe';
}

function simDistStatusPillClass(status){
  return {tossup:'pill-red',battleground:'pill-amber',lean:'pill-green',safe:'pill-blue',absent:'pill-gray'}[status]||'pill-gray';
}

function simDistStatusLabel(status){
  return {tossup:'Critical',battleground:'Contested',lean:'Leads',safe:'Safe',absent:'Weak'}[status]||status;
}

function buildDistrictAdvantageExplanation(distName, d, winner, closeLosers){
  const tot = Object.values(d.v).reduce((a,b)=>a+b,0)||1;
  const sorted = Object.entries(d.v).sort((a,b)=>b[1]-a[1]);
  ...
  const histText = histWins>=3 ? `${winner} has won here in ${histWins}/4 recent elections — a historic stronghold.`
    : histWins===2 ? `${winner} won 2 of the last 4 elections here — established but contested.`
    : `${winner} won here in 2021 but has limited prior history — a recent gain.`;

  // Geographic/demographic
  const geoText = isUrban
    ? `Urban district (${region}) — ${winner} benefits from organised urban networks and educated voter base.`
    : `Rural district (${region}) — ${winner} leverages rural community ties and local notables.`;
  ...
  // Commune breakdown if available
  const commWon = communes.filter(c=>c.w===winner);
  const commText = communes.length > 0
    ? `Wins ${commWon.length}/${communes.length} communes in this district — key strongholds: ${commWon.sort((a,b)=>b.t-a.t).slice(0,3).map(c=>c.n).join(', ')}.`
    : '';

  // Turnout angle
  const toText = to21 < 38 ? `Low turnout district (${to21.toFixed(0)}%) amplifies ${winner}'s organisational advantage — strong ground game converts loyalists reliably.`
    : to21 > 52 ? `High turnout (${to21.toFixed(0)}%) signals strong civic mobilisation — ${winner} benefits from broad name recognition.`
    : `Moderate turnout (${to21.toFixed(0)}%) — competitive ground game is decisive.`;
...
function renderSimulationDetails(){
  ...
  const otherParties = MAJOR.filter(p=>!focusParties.includes(p));

  // Party selector pills at the top (to control status colours)
  const partySelectorHtml = `
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
      <span style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-right:2px">Status per party:</span>
      ${MAJOR.map(p=>`
        <button onclick="simDetailParty='${p}';renderSimulationDetails()" style="padding:4px 12px;font-size:11px;font-weight:700;border-radius:4px;cursor:pointer;font-family:Inter,sans-serif;transition:all .12s;border:2px solid ${pc(p)};background:${simDetailParty===p?pc(p)+'22':'transparent'};color:${pc(p)}">${p}</button>
      `).join('')}
    </div>`;
  ...
  const distCardsHtml = distEntries.map(([name, d])=>{
    const tot = Object.values(d.v).reduce((a,b)=>a+b,0)||1;
    const sorted = Object.entries(d.v).sort((a,b)=>b[1]-a[1]);
    const allWinners = rankBySeats(d.v, d.s);
    const to = TO_MAP[name]||{};
    const to21 = ((to.to21||0.45)*100).toFixed(0);

    // Sim-adjusted winner if sim was run
    const simWinner = simResults&&simResults[name] ? (simResults[name].seats?.[0]||d.w21) : d.w21;

    // Status relative to selected party
    const status = getSimDistStatus(name, d, simDetailParty);
    const pillCls = simDistStatusPillClass(status);
    const pillLbl = simDistStatusLabel(status);

    // Focus party columns: RNI, PAM, PI
    const focusCols = focusParties.map(p=>{
      const v = d.v[p]||0;
      const pct = (v/tot*100).toFixed(1);
      const wins = allWinners.filter(w=>w===p).length;
      return `<div style="text-align:center;padding:0 6px">
        <div style="font-size:11px;font-weight:700;color:${pc(p)}">${pct}%</div>
        <div style="font-size:10px;color:var(--text3)">${wins>0?wins+'s':'—'}</div>
      </div>`;
    }).join('');

    // Other winners (parties NOT in focusParties that won seats)
    const otherWinners = [...new Set(allWinners.filter(p=>!focusParties.includes(p)))];
    const otherChips = otherWinners.map(p=>`
      <span onclick="event.stopPropagation();simDetailParty='${p}';renderSimulationDetails()" title="${p}: ${((d.v[p]||0)/tot*100).toFixed(1)}%" style="display:inline-flex;align-items:center;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;cursor:pointer;background:${pc(p)}18;color:${pc(p)};border:1px solid ${pc(p)}30">${p}</span>
    `).join('');

    return `
    <div class="sim-dist-row" onclick="openSimDistModal('${name.replace(/'/g,"\\'")}')" style="display:grid;grid-template-columns:180px 50px 32px 90px 90px 90px 1fr 80px;align-items:center;gap:0;padding:7px 10px;border-bottom:1px solid var(--border);cursor:pointer;transition:background .1s" onmouseenter="this.style.background='var(--surface2)'" onmouseleave="this.style.background='transparent'">
      <div style="min-width:0">
        ...
        <div style="font-size:10px;color:var(--text3)">${d.r.split(' - ').slice(0,2).join('-').slice(0,22)}</div>
      </div>
      <div style="text-align:center"><span style="font-size:11px;font-weight:700;color:var(--text3)">${d.s}s</span></div>
      <div style="text-align:center"><span style="font-size:10px;color:var(--text3)">${to21}%</span></div>
      ${focusCols}
      <div style="display:flex;gap:4px;flex-wrap:wrap">${otherChips}</div>
      <div style="text-align:right"><span class="pill ${pillCls}">${pillLbl}</span></div>
    </div>`;
  }).join('');
  ...
  <div class="card" style="margin-bottom:12px;padding:14px 16px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
      <div class="card-title" style="margin:0">Simulation Results — All 92 Districts</div>
      <div style="font-size:11px;color:var(--text3)">${simResults&&!simResults._baseline?'Your simulation':'Baseline forecast'} · Click a district to explore</div>
    </div>
    ${partySelectorHtml}
  </div>
  ...
  <div class="card" style="padding:0;overflow:hidden">
    <!-- Header row -->
    <div style="display:grid;grid-template-columns:180px 50px 32px 90px 90px 90px 1fr 80px;align-items:center;gap:0;padding:8px 10px;background:var(--surface2);border-bottom:2px solid var(--border);position:sticky;top:0;z-index:2">
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.07em">District</div>
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-align:center;text-transform:uppercase">Seats</div>
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-align:center;text-transform:uppercase">TO</div>
      ${focusParties.map(p=>`<div style="font-size:10px;font-weight:800;color:${pc(p)};text-align:center">${p}</div>`).join('')}
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase">Other winners</div>
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-align:right;text-transform:uppercase">Status</div>
    </div>
    ${distCardsHtml}
  </div>
  ...
  <div id="sim-dist-modal" onclick="if(event.target===this)closeSimDistModal()" style="display:none;position:fixed;inset:0;z-index:9000;background:rgba(0,0,0,.45);align-items:center;justify-content:center">
    <div id="sim-dist-modal-body" style="background:var(--surface);border-radius:12px;width:580px;max-height:88vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.3)"></div>
  </div>`;
...
function openSimDistModal(distName){
  const d = D[distName];
  if(!d) return;
  const modal = document.getElementById('sim-dist-modal');
  if(!modal) return;
  ...
  const simData = simResults&&simResults[distName];
  const simWinner = simData ? (simData.seats?.[0]||d.w21) : d.w21;
  const winner = uniqueWinners[0]; // rank-1 party

  // Close losers: ranked just below seat cutoff (seats d.s, d.s+1 by votes)
  const closeLosers = sorted.slice(d.s, d.s+2).map(([p])=>p).filter(Boolean);
  // Actually, for "close" we want the parties closest to the winner or to the last seat
  const allRanked = sorted.map(([p,v],i)=>({p,v,pct:(v/tot*100),rank:i+1,wins:allWinners.filter(w=>w===p).length}));
  const firstLoserEntry = allRanked[d.s]; // index d.s = first loser

  const adv = buildDistrictAdvantageExplanation(distName, d, winner, closeLosers);
  const to = TO_MAP[distName]||{};
  ...
  <div style="padding:20px 24px">
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px">
      ...
      <button onclick="closeSimDistModal()" style="font-size:18px;color:var(--text3);background:none;border:none;cursor:pointer;padding:0 0 0 12px;line-height:1">×</button>
    </div>

    <!-- Winner banner -->
    <div style="background:${pc(winner)}12;border:2px solid ${pc(winner)}44;border-radius:8px;padding:14px 16px;margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between">
        <div>
          <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:4px">2021 Rank-1 Winner</div>
        ...
        ${simData?`<div style="text-align:right">
          <div style="font-size:10px;color:var(--text3);margin-bottom:2px">Sim projection</div>
          <div style="font-size:16px;font-weight:800;color:${pc(simWinner)}">${simWinner}</div>
          <div style="font-size:11px;color:var(--text3)">2026 forecast</div>
        </div>`:''}
      </div>
    </div>
    ...
    <div style="margin-bottom:14px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:8px">Seat allocation (${d.s} seats)</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        ${uniqueWinners.map(p=>{
          const seats = allWinners.filter(w=>w===p).length;
          const pct = ((d.v[p]||0)/tot*100).toFixed(1);
          return `<div style="padding:8px 14px;border-radius:7px;background:${pc(p)}15;border:2px solid ${pc(p)}40">
            <div style="font-size:14px;font-weight:900;color:${pc(p)}">${p}</div>
            <div style="font-size:12px;font-weight:700;color:var(--navy)">${seats} seat${seats>1?'s':''}</div>
    ...
    <!-- Full ranking with cutoff line -->
    <div style="margin-bottom:14px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:8px">Full vote ranking</div>
      ${allRanked.slice(0,Math.min(allRanked.length,8)).map((r,i)=>{
        const isSeat = i < d.s;
        const isCutoff = i === d.s;
        const barW = (r.v / sorted[0][1] * 100).toFixed(0);
        const isClose = !isSeat && i <= d.s+1 && ((sorted[d.s-1]?.[1]||0) - r.v)/tot*100 < 8;
        return `${isCutoff?`<div style="display:flex;align-items:center;gap:6px;margin:5px 0;font-size:10px;color:var(--red);font-weight:700"><div style="flex:1;height:1px;background:var(--red)"></div>SEAT CUTOFF<div style="flex:1;height:1px;background:var(--red)"></div></div>`:''}
        <div style="display:flex;align-items:center;gap:8px;padding:5px 0;${isSeat?'':'opacity:.75'}">
          ...
          <span style="font-size:12px;font-weight:${isSeat?'700':'400'};color:${pc(r.p)};min-width:36px">${r.p}</span>
          <div style="flex:1;height:5px;background:var(--border);border-radius:3px">
            <div style="height:100%;width:${barW}%;background:${pc(r.p)};border-radius:3px"></div>
          </div>
          <span style="font-size:11px;min-width:38px;text-align:right">${r.pct.toFixed(1)}%</span>
          <span style="font-size:10px;color:var(--text3);min-width:60px;text-align:right">${r.v.toLocaleString()}</span>
          ${isSeat?`<span style="font-size:10px;color:var(--green);font-weight:700;min-width:42px">✓ SEAT</span>`:
            (isClose?`<span style="font-size:10px;color:var(--red);font-weight:700;min-width:42px">CLOSE</span>`:`<span style="min-width:42px"></span>`)}
        </div>`;
      }).join('')}
    </div>

    <!-- Why the winner won -->
    <div style="background:var(--navy);color:#fff;border-radius:8px;padding:16px;margin-bottom:14px">
    ...
    <div style="margin-bottom:6px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:8px">Winner trajectory 2011–2026</div>
      <div style="display:flex;gap:6px;align-items:center">
        ${['11','15','16','21'].map(yr=>{
          const w=d['w'+yr];
          return w?`<div style="text-align:center">
            <div style="font-size:9px;color:var(--text3);margin-bottom:3px">20${yr}</div>
            <div style="width:38px;height:38px;border-radius:6px;background:${pc(w)}18;border:1px solid ${pc(w)}40;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:${pc(w)}">${w}</div>
          </div>`:'';
        }).filter(Boolean).join('<span style="color:var(--border2);font-size:16px;margin-top:10px">→</span>')}
        ...
        <div style="text-align:center">
          ...
          <div style="width:38px;height:38px;border-radius:6px;background:${pc(simWinner)}22;border:2px solid ${pc(simWinner)};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:${pc(simWinner)}">${simWinner}</div>
        </div>
      </div>
    </div>
  </div>`;
