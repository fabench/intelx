...
function TOTAL(){return Object.values(D).reduce((a,d)=>a+d.s,0);}
...
function setTopTab(tab){
  ...
  if(!selDist||tab==='battleground'||tab==='intel'||tab==='simdetails'||tab==='jsap'||tab==='party'||tab==='scensets'||tab==='h2h'||tab==='strategic'||tab==='comms'||tab==='news'||tab==='orga'){
...
function buildH2HNational(parties){
  ...
  <div style="display:grid;grid-template-columns:${parties.map(()=>'1fr').join(' ')};gap:10px;margin-bottom:14px">
    ${parties.map(p=>{
      ...
      return `<div style="padding:14px;border-radius:8px;border:2px solid ${pc(p)};background:${pc(p)}10">
        ...
        <div style="font-size:32px;font-weight:900;color:var(--navy);line-height:1">${s.seats}<span style="font-size:12px;font-weight:500;color:var(--text3)"> seats</span></div>
        ...
        <div style="margin-top:8px;padding-top:8px;border-top:1px solid ${pc(p)}30;display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:11px">
          <div><div style="color:var(--text3)">Vote share</div><div style="font-weight:700">${s.votePct}%</div></div>
          <div><div style="color:var(--text3)">Districts led</div><div style="font-weight:700">${s.leads}</div></div>
          <div><div style="color:var(--text3)">Seats won</div><div style="font-weight:700">${s.distWon}d</div></div>
          <div><div style="color:var(--text3)">Avg win prob</div><div style="font-weight:700">${s.avgWinProb}%</div></div>
        </div>
      </div>`;
    }).join('')}
  </div>

  <!-- Redesigned strategic insight panel -->
  ...
  <div class="card" style="border-left:3px solid var(--red);margin-bottom:10px">
    <div class="card-title" style="color:var(--red)">⚠ Seat-loss danger zones — last seat within 5pp</div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px">
      ...
      <div style="padding:7px 9px;background:var(--red-bg);border:1px solid #fecaca;border-radius:5px;cursor:pointer" onclick="selectDist('${d.n.replace(/'/g,"\\'")}')">
        ...
        <div style="font-size:10px;color:var(--red);margin-top:2px">Cutoff: ${d.lastSeatWinner}→${d.firstLoser} gap ${d.lastGap.toFixed(1)}pp</div>
      </div>`).join('')}
    </div>
  </div>`:''}

  <!-- Full district table — all 92 -->
  <div class="card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
      <div class="card-title" style="margin:0">All 92 districts — sorted by competitiveness</div>
      <div style="font-size:11px;color:var(--text3)">${contested.length} contested · ${distResults.filter(d=>Object.values(d.pSeats).every(n=>n===0)).length} districts with no selected party winning</div>
    </div>
    <table class="tbl">
      <thead><tr>
        <th>Comp.</th><th>District</th><th>S</th>
        ${parties.map(p=>`<th style="color:${pc(p)}">${p} %</th><th style="color:${pc(p)}">Seats</th><th style="color:${pc(p)}">Win%</th>`).join('')}
        <th>Leader</th><th>Gap</th><th>Status</th>
      </tr></thead>
        ...
        ${parties.map(p=>`
          ...
          <td style="text-align:center">
            <div style="display:flex;align-items:center;gap:3px">
              <div style="flex:1;height:4px;background:var(--border);border-radius:2px;min-width:24px">
                <div style="height:100%;width:${Math.round((d.winProb[p]||0)*100)}%;background:${pc(p)};border-radius:2px"></div>
              </div>
              <span style="font-size:10px;color:${(d.winProb[p]||0)>0.6?pc(p):'var(--text3)'};font-weight:${(d.winProb[p]||0)>0.6?'700':'400'};min-width:28px">${Math.round((d.winProb[p]||0)*100)}%</span>
            </div>
          </td>
        `).join('')}
...
function renderSimulationDetails(){
  ...
  const partySel=`
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
      <span style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-right:2px">Status per:</span>
  ...
  const distEntries=Object.entries(D).sort((a,b)=>a[0].localeCompare(b[0]));
  const rows=distEntries.map(([name,d])=>{
    ...
    return `<div onclick="openSimDistModal('${name.replace(/'/g,"\\'")}')" style="display:grid;grid-template-columns:180px 40px 32px 88px 88px 88px 1fr 80px;align-items:center;padding:7px 10px;border-bottom:1px solid var(--border);cursor:pointer;transition:background .1s" onmouseenter="this.style.background='var(--surface2)'" onmouseleave="this.style.background='transparent'">
  ...
  <div class="card" style="margin-bottom:12px;padding:14px 16px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
      <div class="card-title" style="margin:0">All 92 Districts — ${isCustomSim?'Your Simulation':'Baseline Forecast (10k runs)'}</div>
      <div style="font-size:11px;color:var(--text3)">%/seats/win% per party · Click district to explore</div>
    </div>
    ${partySel}
  </div>
  <div class="card" style="padding:0;overflow:hidden">
    <div style="display:grid;grid-template-columns:180px 40px 32px 88px 88px 88px 1fr 80px;align-items:center;padding:8px 10px;background:var(--surface2);border-bottom:2px solid var(--border);position:sticky;top:0;z-index:2">
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.07em">District</div>
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-align:center">Seats</div>
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-align:center">TO</div>
      ${focusParties.map(p=>`<div style="text-align:center"><div style="font-size:10px;font-weight:800;color:${pc(p)}">${p}</div><div style="font-size:9px;color:var(--text3)">% / s / win%</div></div>`).join('')}
      <div style="font-size:10px;font-weight:700;color:var(--text3)">Others</div>
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-align:right">Status</div>
    </div>
    ${rows}
  </div>
