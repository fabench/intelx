...
// Auto-boot: when app.js finishes loading, init immediately
// (handles the case where login already happened before app.js loaded)
(function(){
  const sk = sessionStorage.getItem('intelx_s');
  const pwd = typeof ACCESS_PWD !== 'undefined' ? ACCESS_PWD : '';
  if(sk && pwd && sk === pwd) {
    // Session valid — hide login overlay and init
