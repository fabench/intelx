// ── STATIC CONFIG ─────────────────────────────────────────────
// Build-time replacements (kept for deploy pipeline compatibility)
...
const PC={RNI:'#4fa8d5',PAM:'#1a2f5e',PI:'#9b59b6',USFP:'#e91e8c',
  MP:'#f0b429',UC:'#e67e22',PPS:'#2980b9',PJD:'#1a237e',JSAP:'#0891b2',MDS:'#059669',DEFAULT:'#94a3b8'};
const MAJOR=['RNI','PAM','PI','USFP','MP','UC','PPS','PJD','MDS'];
const pc=p=>PC[p]||PC.DEFAULT;
const HIST_NAT={
  '2011':{PJD:59,PI:50,RNI:48,PAM:41,USFP:32,MP:29,UC:21,PPS:12,MDS:0},
  '2015':{PAM:55,PJD:50,PI:48,RNI:38,MP:21,UC:16,USFP:16,PPS:5,MDS:0},
  '2016':{PAM:77,PJD:72,RNI:39,PI:39,MP:22,UC:18,USFP:17,PPS:11,MDS:0},
  '2021':{RNI:87,PAM:74,PI:69,USFP:22,MP:20,UC:13,PPS:10,PJD:5,MDS:3}};
...
// ── REGIONAL LISTS (Listes Régionales — 90 seats) ──────────────────────────
const LR={
  'Béni Mellal-Khénifra':  {inscrits:1445275, sieges:7,  region:'Beni-Mellal - Khénifra'},
  'Casablanca-Settat':     {inscrits:3226173, sieges:12, region:'Casablanca - Settat'},
  'Dakhla-Oued Ed-Dahab':  {inscrits:71714,   sieges:3,  region:'Dakhla - Oued Ed-Dahab'},
  'Drâa-Tafilalet':        {inscrits:904329,  sieges:6,  region:'Drâa - Tafilalet'},
  'Fès-Meknès':            {inscrits:2096417, sieges:10, region:'Fès - Meknès'},
  'Guelmim-Oued Noun':     {inscrits:315603,  sieges:5,  region:'Guelmim - Oued-Noun'},
  "L'Oriental":            {inscrits:1340498, sieges:7,  region:'Oriental'},
  'Laâyoune-Sakia El Hamra':{inscrits:235798, sieges:5,  region:'Laâyoune - Sakia-El-Hamra'},
  'Marrakech-Safi':        {inscrits:2324956, sieges:10, region:'Marrakech - Safi'},
  'Rabat-Salé-Kénitra':    {inscrits:2322130, sieges:10, region:'Rabat - Salé - Kénitra'},
  'Souss-Massa':           {inscrits:1558386, sieges:7,  region:'Souss - Massa'},
  'Tanger-Tétouan-Al Hoceima':{inscrits:1734164,sieges:8,region:'Tanger - Tétouan - Al Hoceima'},
};
// Map intelX region names → LR key
const REGION_TO_LR={};
Object.entries(LR).forEach(([lrName,v])=>{REGION_TO_LR[v.region]=lrName;});

// Calculate regional list seats using Hare quota + largest remainder
function calcRegionalSeats(){
  // Aggregate votes per region per party from district data
  const regionVotes={};
  Object.entries(D).forEach(([,d])=>{
    const r=d.r;
    if(!regionVotes[r])regionVotes[r]={};
    Object.entries(d.v).forEach(([p,v])=>{
      regionVotes[r][p]=(regionVotes[r][p]||0)+v;
    });
  });

  const lrResults={};
  Object.entries(LR).forEach(([lrName,{inscrits,sieges,region}])=>{
    const votes=regionVotes[region]||{};
    const totalVotes=Object.values(votes).reduce((a,b)=>a+b,0)||1;
    const quotient=inscrits/sieges; // Hare quota = inscrits / sieges

    // Apply sim adjustments if sim has run
    const adjVotes={};
    Object.entries(votes).forEach(([p,v])=>{
      if(simResults&&!simResults._baseline){
        const sw=(swings[p]||0)/100*totalVotes;
        adjVotes[p]=Math.max(0,v+sw);
      } else {
        adjVotes[p]=v;
      }
    });

    // Allocate seats: each party gets floor(votes/quotient) seats first
    const parties=Object.entries(adjVotes).filter(([,v])=>v>0);
    const initial=parties.map(([p,v])=>({p,v,base:Math.floor(v/quotient),rem:(v/quotient)%1}));
    let allocated=initial.reduce((a,x)=>a+x.base,0);
    let remaining=sieges-allocated;
    // Largest remainder
    const sorted=[...initial].sort((a,b)=>b.rem-a.rem);
    const seatMap={};
    initial.forEach(x=>{seatMap[x.p]=x.base;});
    for(let i=0;i<remaining&&i<sorted.length;i++){seatMap[sorted[i].p]=(seatMap[sorted[i].p]||0)+1;}

    lrResults[lrName]={region,sieges,quotient,votes:adjVotes,seats:seatMap};
  });
  return lrResults;
}

// Aggregate LR seats nationally per party
function getLRNationalSeats(){
  const lr=calcRegionalSeats();
  const totals={};
  Object.values(lr).forEach(({seats})=>{
    Object.entries(seats).forEach(([p,n])=>{totals[p]=(totals[p]||0)+n;});
  });
  return totals;
}

// Combined local + regional seats
function getCombinedSeats(){
  const local=getSeats();
  const regional=getLRNationalSeats();
  const combined={};
  MAJOR.forEach(p=>{combined[p]=(local[p]||0)+(regional[p]||0);});
  return combined;
}

function TOTAL(){return Object.values(D).reduce((a,d)=>a+d.s,0)+Object.values(LR).reduce((a,v)=>a+v.sieges,0);}
function MAJ(){return Math.floor(TOTAL()/2)+1;}
function ALL_REGIONS(){return [...new Set(Object.values(D).map(d=>d.r))].sort();}
...
function getDistSeats(name,d){
  ...
  if(Object.keys(g).length>0){
    ...
    for(const[gp,gn]of Object.entries(g).filter(([,n])=>n>0)){const give=Math.min(gn,seatsLeft);for(let i=0;i<give;i++){alloc.push(gp);}seatsLeft-=give;}
    for(const[p]of ranked){if(seatsLeft<=0)break;alloc.push(p);seatsLeft--;}
    return alloc;
  }
...
function setRegionFilter(region){
  selRegion=region;
  buildSidebar();
  // Propagate filter to whatever tab is currently active
  if(selDist&&D[selDist]&&D[selDist].r!==region&&region!==''){
    selDist=null;
  }
...
function buildSidebar(){
      ...
      ${filteredDists.sort((a,b)=>a.name.localeCompare(b.name)).map(d=>{
        ...
        return `<div class="sb-item${selDist===d.name?' on':''}" onclick="selectDist('${d.name.replace(/'/g,"\\'")}')">
          ...
          <div class="sb-item-right">
            <span style="font-size:10px;font-weight:700;color:var(--navy);background:var(--surface2);border:1px solid var(--border);border-radius:4px;padding:1px 5px">${d.s}s</span>
            <span style="font-size:10px;color:var(--text3);min-width:28px;text-align:right">${((TO_MAP[d.name]||{}).to21||0.45*1)*100>0?Math.round(((TO_MAP[d.name]||{}).to21||0.45)*100)+'%':''}</span>
          </div>
        </div>`;
      }).join('')}
    </div>`;
  ...
  <div style="padding:10px 12px;border-top:1px solid var(--border);margin-top:4px">
    <button onclick="resetSim();goNational();" style="width:100%;padding:7px 0;font-size:11px;font-weight:600;color:var(--text3);background:var(--surface2);border:1px solid var(--border);border-radius:6px;cursor:pointer;font-family:Inter,sans-serif">↺ Reset simulation & clear</button>
  </div>`;
  body.innerHTML=html;
}
...
function setTopTab(tab){
  ...
  if(!selDist||tab==='dashboard'||tab==='battleground'||tab==='intel'||tab==='simdetails'||tab==='lr'||tab==='jsap'||tab==='party'||tab==='scensets'||tab==='h2h'||tab==='strategic'||tab==='comms'||tab==='news'){
    if(tab==='dashboard')renderDashboard();
    else if(tab==='battleground')renderBattlegroundOverview();
    else if(tab==='simdetails')renderSimulationDetails();
    else if(tab==='lr')renderLRView();
...
function selectDist(name){
  selDist=name;
  // Auto-sync right-panel scenario to this district
  if(scenLevel==='district'||selDist)scenDistName=name;
  buildSidebar();
...
function renderSimDistResult(name,d){
  ...
  const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
  const sorted=Object.entries(r.adj||d.v).sort((a,b)=>b[1]-a[1]).slice(0,5);
...
function renderDistHistory(name,d,to){
  const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
  const histYears=['2011','2015','2016','2021'];
  const histWinners=histYears.map(yr=>({yr,w:d['w'+yr.slice(2)],to:to['to'+yr.slice(2)]||0}));
  const winners2021=Object.entries(d.v).sort((a,b)=>b[1]-a[1]).slice(0,d.s);

  // Party colors for history timeline
  const partyTimeline=()=>{
    const parties=[...new Set(histWinners.map(h=>h.w).filter(Boolean))];
    return MAJOR.filter(p=>d.v[p]||parties.includes(p));
  };
  const relevantParties=partyTimeline();

  const html=`
  <!-- Full historical overview -->
  <div class="card" style="margin-bottom:12px">
    <div class="card-title" style="margin-bottom:10px">Trajectoire historique — ${name}</div>
    <!-- Winner timeline -->
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;flex-wrap:wrap">
      ${histWinners.map((h,i)=>`
        <div style="text-align:center">
          <div style="font-size:9px;color:var(--text3);margin-bottom:4px;font-weight:700">${h.yr}</div>
          ${h.w?`<div style="width:46px;height:46px;border-radius:7px;background:${pc(h.w)}22;border:2px solid ${pc(h.w)};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:${pc(h.w)};cursor:pointer" title="${h.w} · TO ${(h.to*100).toFixed(0)}%">${h.w}</div>
          <div style="font-size:9px;color:var(--text3);margin-top:3px">${h.to?(h.to*100).toFixed(0)+'%':''}</div>`
          :`<div style="width:46px;height:46px;border-radius:7px;background:var(--surface2);border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;font-size:9px;color:var(--text4)">—</div>`}
        </div>
        ${i<histWinners.length-1?'<span style="color:var(--border2);font-size:18px;margin-top:-8px">→</span>':''}`).join('')}
      <span style="color:var(--border2);font-size:18px;margin-top:-8px">→</span>
      <div style="text-align:center">
        <div style="font-size:9px;color:var(--teal);margin-bottom:4px;font-weight:700">2026 ↗</div>
        <div style="width:46px;height:46px;border-radius:7px;background:${simResults?pc(getSimWinner(name))+'22':'var(--teal-bg)'};border:2px solid ${simResults?pc(getSimWinner(name)):'var(--teal)'};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:${simResults?pc(getSimWinner(name)):'var(--teal)'}">
    ...
    <!-- Flip indicator -->
    ${d.w11&&d.w21&&d.w11!==d.w21?`<div class="alert al-amber" style="margin-bottom:10px"><span class="al-icon"></span><span>District volatile — a basculé de <strong>${d.w11}</strong> (2011) à <strong>${d.w21}</strong> (2021)</span></div>`:''}
    ${histWinners.filter(h=>h.w).map((h,i,arr)=>{
      if(i===0)return '';
      const prev=arr[i-1];
      if(!prev.w||prev.w===h.w)return '';
      return `<div style="font-size:11px;color:var(--amber);margin-bottom:4px">⚡ Flip ${prev.yr}→${h.yr}: ${prev.w} → ${h.w}</div>`;
    }).join('')}

    <!-- Turnout evolution chart -->
    <div style="margin-bottom:12px">
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Évolution de la participation</div>
      <div style="display:flex;align-items:flex-end;gap:8px;height:50px">
        ${histWinners.filter(h=>h.to>0).map(h=>{
          const maxTo=Math.max(...histWinners.map(x=>x.to||0),0.01);
          const pct=(h.to/maxTo*100).toFixed(0);
          return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px">
            <div style="font-size:9px;color:var(--text3)">${(h.to*100).toFixed(0)}%</div>
            <div style="width:100%;border-radius:3px 3px 0 0;background:${h.w?pc(h.w)+'88':'var(--border)'};height:${pct}%"></div>
            <div style="font-size:9px;color:var(--text3)">${h.yr}</div>
          </div>`;
        }).join('')}
      </div>
    </div>
  ...
  <!-- 2021 vote breakdown -->
  <div class="card" style="margin-bottom:12px">
    <div class="card-title">Résultats 2021 — Répartition des voix (${tot.toLocaleString()} voix exprimées)</div>
    <div class="seat-bar" style="height:12px;margin-bottom:10px">
      ${winners2021.map(([p,v])=>`<div style="width:${(v/tot*100).toFixed(1)}%;background:${pc(p)}" title="${p}: ${v.toLocaleString()}"></div>`).join('')}
    </div>
    ${Object.entries(d.v).sort((a,b)=>b[1]-a[1]).map(([p,v],i)=>{
      const pct=(v/tot*100).toFixed(1);
      const isWinner=i<d.s;
      const simD=simResults&&simResults[name];
      const projV=simD&&simD.adj?Math.round(simD.adj[p]||0):null;
      return `<div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid var(--border)${isWinner?';background:#'+pc(p).replace('#','')+'08':''}">
        <div style="width:7px;height:7px;border-radius:2px;background:${pc(p)};flex-shrink:0"></div>
        <span style="font-size:12px;font-weight:${isWinner?'700':'500'};color:${isWinner?pc(p):'var(--text2)'};min-width:32px">${p}</span>
        <div style="flex:1;height:6px;background:var(--border);border-radius:3px">
          <div style="height:100%;width:${pct}%;background:${pc(p)};border-radius:3px;max-width:100%"></div>
        </div>
        <span style="font-size:11px;min-width:34px;text-align:right">${pct}%</span>
        <span style="font-size:11px;color:var(--text3);min-width:72px;text-align:right">${v.toLocaleString()}</span>
        ${projV!==null?`<span style="font-size:10px;min-width:72px;text-align:right;color:${projV>v?'var(--green)':projV<v?'var(--red)':'var(--text3)'}">${projV>v?'↑':projV<v?'↓':''} ${projV.toLocaleString()} proj.</span>`:''}
        ${isWinner?`<span class="seat-chip" style="background:${pc(p)}">Élu</span>`:''}
      </div>`;
    }).join('')}
    <div style="margin-top:8px;font-size:10px;color:var(--text3)">
      ${d.s} sièges · TO 2021: ${(to.to21||0.45*100)?((to.to21||0.45)*100).toFixed(1)+'%':'—'} · 
      Marge du dernier siège: +${d.m}pp · 
      Jeunes 15–29: ${d.yth}% · ${d.u?'Urbain':'Rural'}
    </div>
  </div>

  <!-- TO progression -->
  <div class="card">
    <div class="card-title">Analyse participation & élasticité</div>
    <div class="score-bar"><span class="score-label">Delta légis–communal</span><div class="score-track"><div class="score-fill" style="width:${Math.min(100,Math.abs(to.delta||0)*200).toFixed(0)}%;background:${(to.delta||0)>0?'var(--green)':'var(--red)'}"></div></div><span class="score-val" style="color:${(to.delta||0)>0?'var(--green)':'var(--red)'}">${((to.delta||0)*100>0?'+':'')+((to.delta||0)*100).toFixed(1)}pp</span></div>
    <div class="score-bar"><span class="score-label">Progression TO 2021</span><div class="score-track"><div class="score-fill" style="width:${Math.min(100,Math.abs(to.prog||0)*100).toFixed(0)}%;background:var(--teal)"></div></div><span class="score-val">${((to.prog||0)*100).toFixed(1)}%</span></div>
    <div class="score-bar"><span class="score-label">Élasticité (sensibilité aux swings)</span><div class="score-track"><div class="score-fill" style="width:${Math.min(100,Math.abs(to.elas||1)/10*100).toFixed(0)}%;background:var(--purple)"></div></div><span class="score-val">${Math.abs(to.elas||1).toFixed(1)}x</span></div>
    <div style="font-size:10px;color:var(--text3);margin-top:8px">
      Voix mobilisables (non-votants): ~${(to.voixAdd||0).toLocaleString()} · Score JSAP: ${(getJSAPscore(d,name)*100).toFixed(0)}%
    </div>
  </div>`;
  document.getElementById('view-content').innerHTML=html;
}
...
function renderNationalOverview(){
  ...
  const localSeats=getSeats();
  const combined=getCombinedSeats();
  const sorted=Object.entries(combined).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
  const total=TOTAL();
  const mx=sorted[0]?.[1]||1;
  const simBannerHtml=simResults?buildSimBanner(combined,sorted):'';
  const scopedD=selRegion?Object.fromEntries(Object.entries(D).filter(([,d])=>d.r===selRegion)):D;
  ...
  <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px;margin-bottom:12px">
    ...
    <div class="card">
      ...
      ${renderCoalition(combined)}
      <div class="divider"></div>
      <div class="card-title" style="margin-bottom:8px">Alertes clés</div>
...
function buildH2HNational(parties){
  ...
  const distResults=Object.entries(D).map(([n,d])=>{
    ...
    // Win probability per party from sim
    const simD=simResults&&simResults[n];
    const winProb={};
    parties.forEach(p=>{
      if(simD&&simD.meanSeats){winProb[p]=Math.min(1,simD.meanSeats[p]||0);}
      else{winProb[p]=pSeats[p]>0?0.9:((d.v[p]||0)>0&&ranked.findIndex(([x])=>x===p)<d.s+1?0.3:0);}
    });
    return {n,d,s:d.s,cs,st:getStatus(d.m,n,d),winners,pSeats,pPct,leader,gap2,seatDanger,lastSeatWinner,firstLoser,lastGap,gapPp:lastGap.toFixed(1),isLastSeatContest,winProb};
  }).sort((a,b)=>a.cs-b.cs); // sorted by competitiveness, most competitive first
  ...
  parties.forEach(p=>{
    h2hStats[p]={
      ...
      distWon:Object.values(D).filter(d=>rankBySeats(d.v,d.s).includes(p)).length,
      dominated:Object.entries(D).filter(([,d])=>rankBySeats(d.v,d.s).every(w=>w===p)).length,
      seatDanger:distResults.filter(dr=>dr.seatDanger&&dr.pSeats[p]===0&&Object.values(dr.pSeats).some(n=>n>0)).length,
      leads:distResults.filter(d=>d.leader===p).length,
      avgWinProb:distResults.length?(distResults.reduce((a,d)=>a+(d.winProb[p]||0),0)/distResults.length*100).toFixed(0):0,
    };
  });

  // Segment districts for insight
  const aVuln=distResults.filter(d=>d.leader===parties[0]&&d.gap2!==null&&parseFloat(d.gap2)<5);
  const bVuln=distResults.filter(d=>d.leader===parties[1]&&d.gap2!==null&&parseFloat(d.gap2)<5);
  const aFlipTargets=distResults.filter(d=>d.leader===parties[1]&&parseFloat(d.gap2||99)<8&&d.pPct[parties[0]]>0).sort((a,b)=>parseFloat(a.gap2)-parseFloat(b.gap2));
  const contested=distResults.filter(d=>d.isLastSeatContest);

  const A=parties[0],B=parties[1],C=parties[2];
  const aSeats=h2hStats[A].seats, bSeats=h2hStats[B].seats;

  const h2hSortFn=(col,dir)=>{
    return (a,b)=>{
      let va,vb;
      if(col==='cs'){va=a.cs;vb=b.cs;}
      else if(col==='name'){va=a.n;vb=b.n;}
      else if(col==='seats'){va=a.s;vb=b.s;}
      else if(col==='gap'){va=parseFloat(a.gap2||99);vb=parseFloat(b.gap2||99);}
      else if(parties.includes(col)){va=a.pPct[col]||0;vb=b.pPct[col]||0;}
      else{va=0;vb=0;}
      return typeof va==='string'?va.localeCompare(vb)*dir:(va-vb)*dir;
    };
  };
  const sortedDists=[...distResults].sort(h2hSortFn(h2hSortCol,h2hSortDir));
  const thSort=(col,label)=>`<th style="cursor:pointer;user-select:none;white-space:nowrap" onclick="h2hSortCol='${col}';h2hSortDir=h2hSortCol==='${col}'?-h2hSortDir:1;updateH2H()">${label}${h2hSortCol===col?(h2hSortDir>0?' ▲':' ▼'):' ↕'}</th>`;
  ...
  <div style="display:grid;grid-template-columns:${parties.map(()=>'1fr').join(' ')};gap:10px;margin-bottom:14px">
    ${parties.map(p=>{
      ...
      const delta=s.seats-b;
      return `<div style="padding:14px;border-radius:8px;border:2px solid ${pc(p)};background:${pc(p)}10">
        <div style="font-size:13px;font-weight:800;color:${pc(p)};margin-bottom:6px">${p}</div>
        <div style="font-size:32px;font-weight:900;color:var(--navy);line-height:1">${s.seats}<span style="font-size:12px;font-weight:500;color:var(--text3)"> sièges</span></div>
        <div style="font-size:11px;color:${delta>0?'var(--green)':delta<0?'var(--red)':'var(--text3)'};font-weight:700;margin-top:3px">${delta>=0?'+':''}${delta} vs 2021</div>
        <div style="margin-top:8px;padding-top:8px;border-top:1px solid ${pc(p)}30;display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:11px">
          <div><div style="color:var(--text3)">Part votes</div><div style="font-weight:700">${s.votePct}%</div></div>
          <div><div style="color:var(--text3)">Districts led</div><div style="font-weight:700">${s.leads}</div></div>
          <div><div style="color:var(--text3)">Sièges gagnés</div><div style="font-weight:700">${s.distWon}d</div></div>
          <div><div style="color:var(--text3)">Prob. moy.</div><div style="font-weight:700">${s.avgWinProb}%</div></div>
        </div>
      </div>`;
    }).join('')}
  </div>

  ${buildH2HInsight(parties,distResults,h2hStats,sim,aVuln,bVuln,aFlipTargets,contested)}

  ${distResults.filter(d=>d.seatDanger).length>0?`
  <div class="card" style="border-left:3px solid var(--red);margin-bottom:10px">
    <div class="card-title" style="color:var(--red)">⚠ Zones de danger — dernier siège à moins de 5pp</div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px">
      ...
      <div style="padding:7px 9px;background:var(--red-bg);border:1px solid #fecaca;border-radius:5px;cursor:pointer" onclick="selectDist('${d.n.replace(/'/g,"\\'")}')">
        ...
        <div style="font-size:10px;color:var(--red);margin-top:2px">Seuil: ${d.lastSeatWinner}→${d.firstLoser} écart ${d.lastGap.toFixed(1)}pp</div>
      </div>`).join('')}
    </div>
  </div>`:''}
  ...
  <div class="card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
      <div class="card-title" style="margin:0">Les 92 circonscriptions — triables par colonne</div>
      <div style="font-size:11px;color:var(--text3)">${contested.length} disputées · Cliquer en-têtes pour trier</div>
    </div>
    <table class="tbl">
      <thead><tr>
        ${thSort('cs','Comp.')}${thSort('name','Circonscription')}${thSort('seats','S')}
        ${parties.map(p=>`${thSort(p,p+' %')}<th style="color:${pc(p)}">Sièges</th><th style="color:${pc(p)}">Voix</th><th style="color:${pc(p)}">Win%</th>`).join('')}
        ${thSort('gap','Écart')}
        <th>Statut</th>
      </tr></thead>
      <tbody>${sortedDists.map(d=>{
        const tot=Object.values(d.d.v).reduce((a,b)=>a+b,0)||1;
        return `<tr onclick="selectDist('${d.n.replace(/'/g,"\\'")}')">
          <td style="font-family:'JetBrains Mono',monospace;font-weight:700;font-size:11px;color:${d.st==='tossup'?'var(--red)':d.st==='battleground'?'var(--amber)':d.st==='lean'?'var(--green)':'var(--text4)'}">${d.cs.toFixed(1)}</td>
          <td style="font-weight:600;font-size:12px">${d.n}</td>
          ...
          ${parties.map(p=>`
            ...
            <td style="text-align:right;font-size:10px;color:var(--text3)">${(d.d.v[p]||0).toLocaleString()}</td>
            <td style="text-align:center">
              <div style="display:flex;align-items:center;gap:3px">
                <div style="flex:1;height:4px;background:var(--border);border-radius:2px;min-width:20px">
                  <div style="height:100%;width:${Math.round((d.winProb[p]||0)*100)}%;background:${pc(p)};border-radius:2px"></div>
                </div>
                <span style="font-size:10px;color:${(d.winProb[p]||0)>0.6?pc(p):'var(--text3)'};font-weight:${(d.winProb[p]||0)>0.6?'700':'400'};min-width:26px">${Math.round((d.winProb[p]||0)*100)}%</span>
              </div>
            </td>
          `).join('')}
          <td style="color:${parseFloat(d.gap2)<3?'var(--red)':parseFloat(d.gap2)<8?'var(--amber)':'var(--text3)'};font-size:11px">${d.gap2!==null?d.gap2+'pp':'—'}</td>
          <td><span class="pill ${statusPillClass(d.st)}">${statusLabel(d.st)}</span></td>
        </tr>`;
      }).join('')}</tbody>
    </table>
...
function buildH2HInsight(parties,distResults,stats,sim,aVuln,bVuln,aFlipTargets,contested){
  const A=parties[0],B=parties[1],C=parties[2];
  const aSeats=stats[A].seats, bSeats=stats[B].seats;
  const diff=aSeats-bSeats;
  const aLeads=stats[A].leads, bLeads=stats[B].leads;
  const cLeads=C?stats[C].leads:0;

  // District-level advantage breakdown
  const aDom=distResults.filter(d=>d.pSeats[A]>0&&d.pSeats[B]===0&&(!C||d.pSeats[C]===0));
  const bDom=distResults.filter(d=>d.pSeats[B]>0&&d.pSeats[A]===0&&(!C||d.pSeats[C]===0));
  const shared=distResults.filter(d=>d.pSeats[A]>0&&d.pSeats[B]>0);
  const aOnly=distResults.filter(d=>d.pSeats[A]>0&&d.pSeats[B]===0);
  const bOnly=distResults.filter(d=>d.pSeats[B]>0&&d.pSeats[A]===0);
  const neitherWins=distResults.filter(d=>Object.values(d.pSeats).every(n=>n===0));

  // Flip opportunities
  const aCanFlip=distResults.filter(d=>d.pSeats[A]===0&&d.pPct[A]>0&&d.lastSeatWinner!==A&&Math.abs((d.d.v[A]||0)-(d.d.v[d.lastSeatWinner]||0))/(Object.values(d.d.v).reduce((a,b)=>a+b,0)||1)*100<6).sort((a,b)=>a.cs-b.cs);
  const bCanFlip=distResults.filter(d=>d.pSeats[B]===0&&d.pPct[B]>0&&d.lastSeatWinner!==B&&Math.abs((d.d.v[B]||0)-(d.d.v[d.lastSeatWinner]||0))/(Object.values(d.d.v).reduce((a,b)=>a+b,0)||1)*100<6).sort((a,b)=>a.cs-b.cs);

  // Seat risk: won districts where winProb <50%
  const aRisk=distResults.filter(d=>d.pSeats[A]>0&&(d.winProb[A]||0)<0.5).sort((a,b)=>(a.winProb[A]||0)-(b.winProb[A]||0));
  const bRisk=distResults.filter(d=>d.pSeats[B]>0&&(d.winProb[B]||0)<0.5).sort((a,b)=>(a.winProb[B]||0)-(b.winProb[B]||0));

  const row=(icon,label,content,color='#cbd5e1')=>`
    <div style="display:flex;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.08)">
      <div style="font-size:14px;flex-shrink:0;width:20px">${icon}</div>
      <div style="flex:1">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:${color};margin-bottom:3px">${label}</div>
        <div style="font-size:12px;line-height:1.6;color:#e2e8f0">${content}</div>
      </div>
    </div>`;

  const chip=(p,txt)=>`<span style="display:inline-block;padding:1px 7px;border-radius:3px;background:${pc(p)}30;color:${pc(p)};font-weight:700;font-size:11px;margin:1px">${txt}</span>`;
  const distChip=(d,extra='')=>`<span onclick="selectDist('${d.n.replace(/'/g,"\\'")}');event.stopPropagation()" style="cursor:pointer;display:inline-block;padding:1px 7px;border-radius:3px;background:rgba(255,255,255,.08);color:#e2e8f0;font-size:11px;margin:1px;text-decoration:underline dotted">${d.n.split(' - ')[0].split(' ')[0]}${extra}</span>`;

  let html=`<div style="padding:14px 16px;background:var(--navy);border-radius:8px;margin-bottom:12px">
    <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#a8d4e8;margin-bottom:10px">Strategic Intelligence Brief — ${A}${B?' vs '+B:''}${C?' vs '+C:''}</div>`;

  // SECTION 1: Overall balance
  html+=row('⚖️','Overall balance',
    `${chip(A,A+' '+aSeats+'s')} ${diff>0?'leads by <strong>'+diff+'</strong> seats':diff<0?'trails by <strong>'+Math.abs(diff)+'</strong> seats':'<strong>tied</strong>'} vs ${chip(B,B+' '+bSeats+'s')}${C?' · '+chip(C,C+' '+(stats[C].seats)+'s'):''}. `+
    `${A} controls <strong>${aLeads}</strong> districts, ${B} <strong>${bLeads}</strong>${C?' , '+C+' <strong>'+cLeads+'</strong>':''}.`,
    '#93c5fd');

  // SECTION 2: Exclusivity — who's where without the other
  html+=row('🗺️','Geographic split',
    `${chip(A,A)} alone wins <strong>${aOnly.length}</strong> districts ${aOnly.slice(0,3).map(d=>distChip(d)).join(' ')}. `+
    `${chip(B,B)} alone wins <strong>${bOnly.length}</strong> districts ${bOnly.slice(0,3).map(d=>distChip(d)).join(' ')}. `+
    `Both parties win seats together in <strong>${shared.length}</strong> districts.`);

  // SECTION 3: Seat risk
  if(aRisk.length||bRisk.length){
    html+=row('🔴','Seat risk',
      (aRisk.length?`${chip(A,A)} has <strong>${aRisk.length}</strong> seats at risk (win prob <50%): ${aRisk.slice(0,3).map(d=>distChip(d,' '+Math.round((d.winProb[A]||0)*100)+'%')).join(' ')}. `:'No '+A+' seats at risk. ')+
      (bRisk.length?`${chip(B,B)} has <strong>${bRisk.length}</strong> seats at risk: ${bRisk.slice(0,3).map(d=>distChip(d,' '+Math.round((d.winProb[B]||0)*100)+'%')).join(' ')}.`:'No '+B+' seats at risk.'),
      '#fca5a5');
  }

  // SECTION 4: Flip opportunities
  if(aCanFlip.length||bCanFlip.length){
    html+=row('🎯','Flip opportunities',
      (aCanFlip.length?`${chip(A,A)} can realistically flip <strong>${aCanFlip.length}</strong> seats — top targets: ${aCanFlip.slice(0,3).map(d=>distChip(d,' '+d.cs.toFixed(1))).join(' ')}. `:'')+ 
      (bCanFlip.length?`${chip(B,B)} can flip <strong>${bCanFlip.length}</strong>: ${bCanFlip.slice(0,3).map(d=>distChip(d,' '+d.cs.toFixed(1))).join(' ')}.`:''),
      '#86efac');
  }

  // SECTION 5: Contested battleground
  if(contested.length){
    const small=contested.filter(d=>d.s<=3);
    html+=row('⚡','Contested seats',
      `<strong>${contested.length}</strong> districts have at least one selected party competing for the last seat. `+
      (small.length?`<strong>${small.length}</strong> are 2–3 seat districts where margins are razor thin: ${small.slice(0,4).map(d=>distChip(d,' '+d.lastGap.toFixed(1)+'pp')).join(' ')}.`:''),
      '#fcd34d');
  }

  // SECTION 6: C-party if present
  if(C){
    const cRisk=distResults.filter(d=>d.pSeats[C]>0&&(d.winProb[C]||0)<0.5);
    const cFlip=distResults.filter(d=>d.pSeats[C]===0&&d.pPct[C]>0&&Math.abs((d.d.v[C]||0)-(d.d.v[d.lastSeatWinner]||0))/(Object.values(d.d.v).reduce((a,b)=>a+b,0)||1)*100<6);
    html+=row('◆',C+' factor',
      `${chip(C,C+' '+stats[C].seats+'s')} wins <strong>${distResults.filter(d=>d.pSeats[C]>0).length}</strong> districts. `+
      (cRisk.length?`<strong>${cRisk.length}</strong> seats at risk. `:'')+
      (cFlip.length?`Can flip <strong>${cFlip.length}</strong> additional districts.`:''));
  }
...
function renderDashboard(){
  document.getElementById('view-tabs').innerHTML='';
  document.getElementById('rp-head').textContent='intelX — Morocco 2026';
  const local=getSeats();
  const regional=getLRNationalSeats();
  const combined=getCombinedSeats();
  const sortedComb=Object.entries(combined).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
  const total=TOTAL();
  const maj=MAJ();
  const leader=sortedComb[0]?.[0]||'—';
  const leaderN=sortedComb[0]?.[1]||0;
  const tossups=Object.entries(D).filter(([n,d])=>getStatus(d.m,n,d)==='tossup');
  const tossupSeats=tossups.reduce((a,[,d])=>a+d.s,0);
  const isCustomSim=simResults&&!simResults._baseline;

  // Region filter: scope data if region selected
  const scopedD=selRegion?Object.fromEntries(Object.entries(D).filter(([,d])=>d.r===selRegion)):D;
  const scopedDistCount=Object.keys(scopedD).length;
  const scopedSeats=Object.values(scopedD).reduce((a,d)=>a+d.s,0);

  // Top competitive districts (scoped)
  const topCompet=Object.entries(scopedD).map(([n,d])=>({n,d,cs:compScore(n,d),st:getStatus(d.m,n,d)}))
    .sort((a,b)=>a.cs-b.cs).slice(0,6);

  // Party performance in scoped area
  const scopedVotes={};
  let scopedTot=0;
  Object.values(scopedD).forEach(d=>{
    const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||0;
    scopedTot+=tot;
    Object.entries(d.v).forEach(([p,v])=>{scopedVotes[p]=(scopedVotes[p]||0)+v;});
  });

  // Recent sim votes — aggregate projected votes from sim
  const simVotes={};
  if(simResults){
    MAJOR.forEach(p=>{
      let vTotal=0;
      Object.entries(scopedD).forEach(([name,d])=>{
        const simD=simResults[name];
        const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
        if(simD&&simD.adj){vTotal+=(simD.adj[p]||0);}
        else{
          const effTo=simD?1:1;
          vTotal+=(d.v[p]||0);
        }
      });
      simVotes[p]=Math.round(vTotal);
    });
  }

  const html=`
  <!-- Top KPI strip -->
  <div class="stat-grid" style="grid-template-columns:repeat(6,1fr);margin-bottom:14px">
    <div class="stat-box" style="background:var(--navy);border-color:var(--navy)">
      <div class="stat-label" style="color:#a8d4e8">Total seats</div>
      <div class="stat-val" style="color:#fff">${total}</div>
      <div class="stat-sub" style="color:#a8d4e8">305 local + 90 régionales</div>
    </div>
    <div class="stat-box" style="background:var(--navy);border-color:var(--navy)">
      <div class="stat-label" style="color:#a8d4e8">Majorité</div>
      <div class="stat-val" style="color:#fff">${maj}</div>
      <div class="stat-sub" style="color:#a8d4e8">sièges requis</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">Leader projeté</div>
      <div class="stat-val" style="color:${pc(leader)}">${leader}</div>
      <div class="stat-sub">${leaderN} sièges combinés</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">Majorité?</div>
      <div class="stat-val" style="color:${leaderN>=maj?'var(--green)':'var(--red)'}">
        ${leaderN>=maj?'✓ OUI':'✗ NON'}
      </div>
      <div class="stat-sub">${leaderN>=maj?'+'+( leaderN-maj)+' au-dessus':maj-leaderN+' manquants'}</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">Toss-ups</div>
      <div class="stat-val" style="color:var(--red)">${tossups.length}</div>
      <div class="stat-sub">${tossupSeats} sièges &lt;3pp</div>
    </div>
    <div class="stat-box" style="background:${isCustomSim?'var(--teal-bg)':'var(--surface2)'}">
      <div class="stat-label">Simulation</div>
      <div class="stat-val" style="font-size:14px;color:${isCustomSim?'var(--teal)':'var(--text3)'}">
        ${isCustomSim?'Active':'Baseline'}
      </div>
      <div class="stat-sub">${isCustomSim?(simResults._runs||0).toLocaleString()+' runs':'Prévision 10k runs'}</div>
    </div>
  </div>

  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">

    <!-- Combined seat projection -->
    <div class="card">
      <div class="card-title" style="display:flex;justify-content:space-between">
        <span>Projection combinée — Locales + Régionales</span>
        <span style="font-size:10px;color:var(--teal);font-weight:600;cursor:pointer" onclick="setTopTab('forecast')">Locales →</span>
      </div>
      <div class="seat-bar" style="height:20px;margin-bottom:10px">
        ${sortedComb.map(([p,n])=>`<div style="width:${(n/total*100).toFixed(1)}%;background:${pc(p)}" title="${p}: ${n}"></div>`).join('')}
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:3px">
        ${sortedComb.filter(([,v])=>v>0).map(([p,n])=>{
          const loc=local[p]||0; const reg=regional[p]||0;
          const b=HIST_NAT['2021'][p]||0; const delta=loc-b;
          return `<div style="display:flex;align-items:center;gap:5px;padding:4px 6px;border-radius:5px;background:${pc(p)}08">
            <div style="width:8px;height:8px;border-radius:2px;background:${pc(p)};flex-shrink:0"></div>
            <span style="font-size:11px;font-weight:700;color:${pc(p)};min-width:28px">${p}</span>
            <span style="font-size:14px;font-weight:800;color:var(--navy);min-width:24px">${n}</span>
            <span style="font-size:9px;color:var(--text3)">${loc}L+${reg}R</span>
            <span style="font-size:10px;font-weight:600;color:${delta>0?'var(--green)':delta<0?'var(--red)':'var(--text3)'}; margin-left:auto">${delta>=0?'+':''}${delta}</span>
          </div>`;
        }).join('')}
      </div>
      <div style="margin-top:10px;padding-top:8px;border-top:1px solid var(--border);display:flex;gap:8px">
        <div style="flex:1;text-align:center;padding:6px;background:var(--surface2);border-radius:5px">
          <div style="font-size:10px;color:var(--text3)">Locales</div>
          <div style="font-size:16px;font-weight:800;color:var(--navy)">305</div>
        </div>
        <div style="flex:1;text-align:center;padding:6px;background:var(--teal-bg);border-radius:5px;cursor:pointer" onclick="setTopTab('lr')">
          <div style="font-size:10px;color:var(--teal)">Régionales</div>
          <div style="font-size:16px;font-weight:800;color:var(--teal)">90</div>
        </div>
        <div style="flex:1;text-align:center;padding:6px;background:${leaderN>=maj?'var(--green-bg)':'var(--red-bg)'};border-radius:5px">
          <div style="font-size:10px;color:${leaderN>=maj?'var(--green)':'var(--red)'}">Majorité</div>
          <div style="font-size:16px;font-weight:800;color:${leaderN>=maj?'var(--green)':'var(--red)'}">${maj}</div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-title">Coalitions possibles</div>
      ${(()=>{
        const s=sortedComb;
        if(!s.length)return '';
        const [lp,ln]=s[0]; const s2=s[1]; const s3=s[2]; const s4=s[3];
        const scenarios=[
          {l:`${lp} seul`,n:ln,col:lp},
          s2?{l:`${lp} + ${s2[0]}`,n:ln+s2[1],col:lp}:null,
          s2&&s3?{l:`${lp} + ${s2[0]} + ${s3[0]}`,n:ln+s2[1]+s3[1],col:lp}:null,
          s2&&s3&&s4?{l:`+ ${s4[0]}`,n:ln+s2[1]+s3[1]+s4[1],col:lp}:null,
        ].filter(Boolean);
        return scenarios.map(sc=>{
          ...
          return `<div class="coal-row">
            <span class="coal-n" style="color:${ok?'var(--green)':'var(--navy)'}">${sc.n}</span>
            <div class="coal-bar"><div class="coal-fill" style="width:${pct.toFixed(0)}%;background:${ok?'var(--green)':'var(--border2)'}"></div></div>
            <span class="coal-lbl">${sc.l}</span>
            <span class="coal-st" style="color:${ok?'var(--green)':'var(--red)'}">${ok?'MAJ':'−'+(maj-sc.n)}</span>
          </div>`;
        }).join('');
      })()}
    </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:12px">
    ...
    <!-- Top competitive districts -->
    <div class="card">
      <div class="card-title" style="display:flex;justify-content:space-between">
        <span>Circonscriptions clés${selRegion?' — '+selRegion.split(' - ')[0]:''}</span>
        <span style="font-size:10px;color:var(--teal);cursor:pointer" onclick="setTopTab('battleground')">Voir tout →</span>
      </div>
      ${topCompet.map(({n,d,cs,st})=>{
        const allW=rankBySeats(d.v,d.s);
        const simD=simResults&&simResults[n];
        const proj=simD?(simD.seats?.[0]||d.w21):d.w21;
        return `<div style="display:flex;align-items:center;gap:6px;padding:6px 0;border-bottom:1px solid var(--border);cursor:pointer" onclick="selectDist('${n.replace(/'/g,"\\'")}')">
          <span class="pill ${statusPillClass(st)}" style="font-size:9px;min-width:24px">${cs.toFixed(1)}</span>
          <div style="flex:1;min-width:0">
            <div style="font-size:11px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${n}</div>
            <div style="font-size:10px;color:var(--text3)">${d.s}s · +${d.m}pp</div>
          </div>
          <span style="font-size:10px;font-weight:700;color:${pc(proj)}">${proj}</span>
        </div>`;
      }).join('')}
    </div>

    <!-- Projected votes by party (scoped) -->
    <div class="card">
      <div class="card-title">Voix projetées${selRegion?' — '+selRegion.split(' - ')[0]:' — National'}</div>
      ${Object.entries(scopedVotes).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([p,v])=>{
        const pct=(v/scopedTot*100).toFixed(1);
        const simV=simVotes[p];
        const delta=simV?Math.round(simV-v):null;
        return `<div style="display:flex;align-items:center;gap:6px;padding:3px 0">
          ...
          <span style="font-size:10px;min-width:34px;text-align:right">${pct}%</span>
          <span style="font-size:10px;color:var(--text3);min-width:62px;text-align:right">${v.toLocaleString()}</span>
          ${delta!==null&&isCustomSim?`<span style="font-size:9px;color:${delta>0?'var(--green)':'var(--red)'};min-width:44px">${delta>=0?'+':''}${delta.toLocaleString()}</span>`:''}
        </div>`;
      }).join('')}
    </div>
    ...
    <!-- Alerts & key signals -->
    <div class="card">
      <div class="card-title">Signaux clés</div>
      ${renderAlerts(combined)}
      <div class="divider"></div>
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Actions rapides</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:5px">
        ${['forecast','lr','h2h','simdetails','party','strategic'].map((t,i)=>{
          const labels={forecast:'Forecast',lr:'Listes Rég.',h2h:'Head-to-Head',simdetails:'Sim Details',party:'Party Profile',strategic:'Strategic'};
          return `<button onclick="setTopTab('${t}')" style="padding:7px 4px;font-size:11px;font-weight:600;background:var(--surface2);border:1px solid var(--border);border-radius:5px;cursor:pointer;font-family:Inter,sans-serif;color:var(--navy)">${labels[t]}</button>`;
        }).join('')}
      </div>
    </div>
  </div>
  ...
  <div class="card" style="margin-top:12px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px">
      <div class="card-title" style="margin:0">Analyse visuelle — Performance des partis</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        <select id="dash-chart-filter" onchange="renderDashboardCharts()" style="font-size:11px;padding:4px 8px;border:1px solid var(--border);border-radius:4px;font-family:Inter,sans-serif;background:var(--surface2)">
          <option value="national">National</option>
          ${selRegion?`<option value="region" selected>${selRegion.split(' - ')[0]}</option>`:Object.keys({...Object.fromEntries(Object.values(D).map(d=>[d.r,1]))}).map(r=>`<option value="${r}">${r.split(' - ')[0]}</option>`).join('')}
        </select>
        <select id="dash-chart-type" onchange="renderDashboardCharts()" style="font-size:11px;padding:4px 8px;border:1px solid var(--border);border-radius:4px;font-family:Inter,sans-serif;background:var(--surface2)">
          <option value="seats">Sièges 2021 vs 2026</option>
          <option value="votes">Part des voix</option>
          <option value="delta">Δ Évolution</option>
        </select>
      </div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      <div>
        <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Sièges — Comparaison 2021 / 2026</div>
        <div style="height:200px;position:relative"><canvas id="dash-chart-seats"></canvas></div>
      </div>
      <div>
        <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Part des voix 2021</div>
        <div style="height:200px;position:relative"><canvas id="dash-chart-votes"></canvas></div>
      </div>
    </div>
  </div>`;
...
function renderDashboardCharts(){
  ...
  const combined=getCombinedSeats();
  const hist=HIST_NAT;
  const chartFilter=document.getElementById('dash-chart-filter')?.value||'national';
  const scopedD=chartFilter!=='national'
    ? Object.fromEntries(Object.entries(D).filter(([,d])=>d.r===chartFilter))
    : (selRegion?Object.fromEntries(Object.entries(D).filter(([,d])=>d.r===selRegion)):D);
  const parties=MAJOR.filter(p=>(combined[p]||0)>0||(hist['2021'][p]||0)>0);

  // Chart 1: Seats 2021 vs 2026
  const c1=document.getElementById('dash-chart-seats');
  if(c1){
    if(c1._ci)c1._ci.destroy();
    // Compute local seats by party for scoped districts
    const localScoped={};MAJOR.forEach(p=>{localScoped[p]=0;});
    Object.entries(scopedD).forEach(([n,d])=>{
      const w=simResults?getSimWinner(n):d.w21;
      if(w)localScoped[w]=(localScoped[w]||0)+1;
    });
    const hist21Scoped={};MAJOR.forEach(p=>{hist21Scoped[p]=0;});
    Object.values(scopedD).forEach(d=>{if(d.w21)hist21Scoped[d.w21]=(hist21Scoped[d.w21]||0)+1;});
    const filtParties=parties.filter(p=>(localScoped[p]||0)>0||(hist21Scoped[p]||0)>0);
    c1._ci=new Chart(c1,{
      type:'bar',
      data:{
        labels:filtParties,
        datasets:[
          {label:'2021',data:filtParties.map(p=>hist21Scoped[p]||0),backgroundColor:filtParties.map(p=>pc(p)+'55'),borderColor:filtParties.map(p=>pc(p)),borderWidth:1.5,borderRadius:3},
          {label:'2026 proj.',data:filtParties.map(p=>localScoped[p]||0),backgroundColor:filtParties.map(p=>pc(p)),borderColor:filtParties.map(p=>pc(p)),borderWidth:1,borderRadius:3}
        ]
      },
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{font:{size:10},color:'#4a5a6b',boxWidth:10}}},scales:{y:{beginAtZero:true,ticks:{font:{size:10},color:'#8a96a3'},grid:{color:'#dde2e811'}},x:{ticks:{font:{size:10},color:'#4a5a6b'},grid:{display:false}}}}
    });
  }

  // Chart 2: Vote share doughnut
  const c2=document.getElementById('dash-chart-votes');
  if(c2){
    if(c2._ci)c2._ci.destroy();
    const voteByParty={};let totalV=0;
    Object.values(scopedD).forEach(d=>{Object.entries(d.v).forEach(([p,v])=>{voteByParty[p]=(voteByParty[p]||0)+v;totalV+=v;});});
    const top=Object.entries(voteByParty).sort((a,b)=>b[1]-a[1]).slice(0,9);
    c2._ci=new Chart(c2,{
      type:'doughnut',
      data:{
        labels:top.map(([p])=>p),
        datasets:[{data:top.map(([,v])=>+(v/totalV*100).toFixed(1)),backgroundColor:top.map(([p])=>pc(p)+'cc'),borderWidth:1.5,borderColor:top.map(([p])=>pc(p))}]
      },
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{font:{size:10},color:'#4a5a6b',padding:5,boxWidth:10}},tooltip:{callbacks:{label:ctx=>`${ctx.label}: ${ctx.parsed}%`}}}}
    });
  }
}


let lrSortCol='region'; let lrSortDir=1;

function renderLRView(){
  document.getElementById('view-tabs').innerHTML='';
  document.getElementById('rp-head').textContent='Listes Régionales';
  const lrData=calcRegionalSeats();

  // National LR totals per party
  const lrNat={};
  MAJOR.forEach(p=>{lrNat[p]=Object.values(lrData).reduce((a,{seats})=>a+(seats[p]||0),0);});
  const lrSorted=Object.entries(lrNat).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);

  // Filter by selected region
  const scopedLR=selRegion
    ? Object.fromEntries(Object.entries(lrData).filter(([,{region}])=>region===selRegion))
    : lrData;

  const rows=Object.entries(scopedLR).map(([lrName,{region,sieges,quotient,votes,seats}])=>{
    const totalV=Object.values(votes).reduce((a,b)=>a+b,0)||1;
    const lrReg=LR[lrName];
    return {lrName,region,sieges,quotient,votes,seats,totalV,inscrits:lrReg?.inscrits||0};
  }).sort((a,b)=>{
    const va=lrSortCol==='region'?a.region:lrSortCol==='sieges'?a.sieges:lrSortCol==='inscrits'?a.inscrits:0;
    const vb=lrSortCol==='region'?b.region:lrSortCol==='sieges'?b.sieges:lrSortCol==='inscrits'?b.inscrits:0;
    return typeof va==='string'?va.localeCompare(vb)*lrSortDir:(va-vb)*lrSortDir;
  });

  const sortBtn=(col,label)=>`<th style="cursor:pointer;user-select:none" onclick="lrSortCol='${col}';lrSortDir=lrSortCol==='${col}'?-lrSortDir:1;renderLRView()">${label}${lrSortCol===col?(lrSortDir>0?' ▲':' ▼'):' ↕'}</th>`;

  const html=`
  <!-- National LR summary -->
  <div class="card" style="margin-bottom:12px">
    <div class="card-title">Répartition nationale — Listes Régionales (90 sièges · Quotient de Hare + Plus Fort Reste)</div>
    <div class="seat-bar" style="height:18px;margin-bottom:8px">
      ${lrSorted.map(([p,n])=>`<div style="width:${(n/90*100).toFixed(1)}%;background:${pc(p)}" title="${p}: ${n}"></div>`).join('')}
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      ${lrSorted.map(([p,n])=>`<div style="display:flex;align-items:center;gap:4px">
        <div style="width:8px;height:8px;border-radius:2px;background:${pc(p)}"></div>
        <span style="font-size:11px;font-weight:700;color:${pc(p)}">${p}</span>
        <span style="font-size:13px;font-weight:800;color:var(--navy)">${n}</span>
        <span style="font-size:10px;color:var(--text3)">sièges</span>
      </div>`).join('')}
    </div>
    <div style="margin-top:8px;font-size:10px;color:var(--text3)">
      Méthode: Quotient = Inscrits ÷ Sièges par région. Attribution: sièges entiers d'abord, puis plus forts restes.
      ${simResults&&!simResults._baseline?'<span style="color:var(--teal);font-weight:600"> · Simulation active — swings appliqués aux voix régionales</span>':''}
    </div>
  </div>
  ...
  <div class="card" style="padding:0;overflow:hidden">
    <table class="tbl">
      <thead><tr>
        ${sortBtn('region','Région')}
        ${sortBtn('inscrits','Inscrits')}
        ${sortBtn('sieges','Sièges')}
        <th>Quotient</th>
        ${MAJOR.map(p=>`<th style="color:${pc(p)};cursor:pointer" onclick="lrSortCol='${p}';lrSortDir*=-1;renderLRView()">${p}${lrSortCol===p?(lrSortDir>0?' ▲':' ▼'):''}</th>`).join('')}
        <th>Gagnant</th>
      </tr></thead>
      <tbody>
        ${rows.map(({lrName,region,sieges,quotient,votes,seats,totalV,inscrits})=>{
          const winner=Object.entries(seats).sort((a,b)=>b[1]-a[1])[0];
          return `<tr>
            <td style="font-weight:700">${lrName}</td>
            <td style="font-family:'JetBrains Mono',monospace;font-size:11px">${inscrits.toLocaleString()}</td>
            <td style="text-align:center;font-weight:700">${sieges}</td>
            <td style="font-family:'JetBrains Mono',monospace;font-size:11px">${Math.round(quotient).toLocaleString()}</td>
            ${MAJOR.map(p=>{
              const v=votes[p]||0;
              const s=seats[p]||0;
              const pct=(v/totalV*100).toFixed(1);
              return `<td style="text-align:center">
                ${v>0?`<div style="font-size:11px;font-weight:${s>0?'700':'400'};color:${s>0?pc(p):'var(--text3)'}">${pct}%</div>
                <div style="font-size:10px;color:${s>0?pc(p):'var(--text4)'}">${s>0?s+'s':'—'}</div>`:'<span style="color:var(--text4)">—</span>'}
              </td>`;
            }).join('')}
            <td>${winner?`<span style="font-weight:700;color:${pc(winner[0])}">${winner[0]} <span style="font-size:10px">(${winner[1]}s)</span></span>`:'—'}</td>
          </tr>`;
        }).join('')}
      </tbody>
    </table>
  ...
  <!-- Per-region detailed breakdown -->
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px">
    ${rows.map(({lrName,region,sieges,quotient,votes,seats,totalV,inscrits})=>{
      const ranked=Object.entries(votes).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
      const winner=Object.entries(seats).sort((a,b)=>b[1]-a[1])[0];
      return `<div class="card" style="padding:12px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
          <div>
            <div style="font-size:12px;font-weight:800;color:var(--navy)">${lrName}</div>
            <div style="font-size:10px;color:var(--text3)">${inscrits.toLocaleString()} inscrits · ${sieges} sièges · Quotient: ${Math.round(quotient).toLocaleString()}</div>
          </div>
          ${winner?`<span style="font-size:11px;font-weight:700;color:${pc(winner[0])}">${winner[0]} leads</span>`:''}
        </div>
        <div class="seat-bar" style="height:10px;margin-bottom:8px">
          ${Object.entries(seats).filter(([,n])=>n>0).sort((a,b)=>b[1]-a[1]).map(([p,n])=>`<div style="width:${(n/sieges*100).toFixed(1)}%;background:${pc(p)}" title="${p}: ${n}s"></div>`).join('')}
        </div>
        ${ranked.slice(0,5).map(([p,v])=>{
          const pct=(v/totalV*100).toFixed(1);
          const s=seats[p]||0;
          const quot=v/quotient;
          return `<div style="display:flex;align-items:center;gap:5px;padding:2px 0;font-size:11px">
            <div style="width:7px;height:7px;border-radius:2px;background:${pc(p)};flex-shrink:0"></div>
            <span style="font-weight:700;color:${pc(p)};min-width:28px">${p}</span>
            <div style="flex:1;height:4px;background:var(--border);border-radius:2px">
              <div style="height:100%;width:${pct}%;background:${pc(p)};border-radius:2px;max-width:100%"></div>
            </div>
            <span style="min-width:36px;text-align:right">${pct}%</span>
            <span style="min-width:52px;text-align:right;color:var(--text3)">${v.toLocaleString()}</span>
            <span style="min-width:28px;font-weight:700;color:${s>0?pc(p):'var(--text4)'}">${s>0?s+'s':'—'}</span>
            <span style="min-width:36px;font-size:10px;color:var(--text3)">(Q${quot.toFixed(2)})</span>
          </div>`;
        }).join('')}
      </div>`;
    }).join('')}
...
let simDetailParty='RNI';
let h2hSortCol='cs'; let h2hSortDir=1;
let simDetailSortCol='name'; let simDetailSortDir=1;

function getSimDistStatus(distName,d,party){
  const allWinners=rankBySeats(d.v,d.s);
  const partyWins=allWinners.includes(party);
  const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
  const sorted=Object.entries(d.v).sort((a,b)=>b[1]-a[1]);
  const partyVotes=d.v[party]||0;
  if(!partyWins){
    const lastWinner=sorted[d.s-1];
    const gap=lastWinner?(lastWinner[1]-partyVotes)/tot*100:100;
    if(gap<3)return'tossup';
    if(gap<8)return'battleground';
    return'absent';
  }
  const simD=simResults&&simResults[distName];
  const meanSeats=simD?simD.meanSeats||{}:{};
  const simChance=meanSeats[party]!==undefined?meanSeats[party]:(partyWins?0.9:0);
  if(simChance<0.35)return'tossup';
  if(simChance<0.6)return'battleground';
  if(simChance<0.85)return'lean';
  return'safe';
}

function simDistPillClass(s){return{tossup:'pill-red',battleground:'pill-amber',lean:'pill-green',safe:'pill-blue',absent:'pill-gray'}[s]||'pill-gray';}
function simDistLabel(s){return{tossup:'Critical',battleground:'Contested',lean:'Leads',safe:'Safe',absent:'Weak'}[s]||s;}

function buildDistAdvantage(distName,d,winner,closeLosers){
  const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
  const sorted=Object.entries(d.v).sort((a,b)=>b[1]-a[1]);
  const winnerVotes=d.v[winner]||0;
  const winnerPct=(winnerVotes/tot*100).toFixed(1);
  const to=TO_MAP[distName]||{};
  const to21=((to.to21||0.45)*100);
  const communes=COMMUNES[distName]||[];
  const histWins=['11','15','16','21'].filter(y=>d['w'+y]===winner).length;
  const histText=histWins>=3?`${winner} has won here in ${histWins}/4 recent elections — a historic stronghold.`:histWins===2?`${winner} won 2 of the last 4 elections here — established but contested.`:`${winner} won here in 2021 but has limited prior history — a recent gain.`;
  const geoText=d.u?`Urban district (${d.r}) — ${winner} benefits from organised urban networks and educated voter base.`:`Rural district (${d.r}) — ${winner} leverages rural community ties and local notables.`;
  const runner=closeLosers[0];
  const gapPp=runner?((winnerVotes-(d.v[runner]||0))/tot*100).toFixed(1):null;
  const leadText=gapPp?`${winner} leads runner-up ${runner} by ${gapPp}pp (${winnerPct}% vs ${((d.v[runner]||0)/tot*100).toFixed(1)}%).`:`${winner} holds ${winnerPct}% of the vote.`;
  const commWon=communes.filter(c=>c.w===winner);
  const commText=communes.length>0?`Wins ${commWon.length}/${communes.length} communes — key strongholds: ${commWon.sort((a,b)=>b.t-a.t).slice(0,3).map(c=>c.n).join(', ')}.`:'';
  const toText=to21<38?`Low turnout (${to21.toFixed(0)}%) amplifies ${winner}'s organisational advantage — strong ground game converts loyalists reliably.`:to21>52?`High turnout (${to21.toFixed(0)}%) signals strong civic mobilisation — ${winner} benefits from broad name recognition.`:`Moderate turnout (${to21.toFixed(0)}%) — competitive ground game is decisive.`;
  return{histText,geoText,leadText,commText,toText};
}

function renderSimulationDetails(){
  document.getElementById('view-tabs').innerHTML='';
  document.getElementById('rp-head').textContent='Simulation Details';
  const focusParties=['RNI','PAM','PI'];
  const partySel=`
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
      <span style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-right:2px">Statut par:</span>
      ${MAJOR.map(p=>`<button onclick="simDetailParty='${p}';renderSimulationDetails()" style="padding:3px 11px;font-size:11px;font-weight:700;border-radius:4px;cursor:pointer;font-family:Inter,sans-serif;transition:all .12s;border:2px solid ${pc(p)};background:${simDetailParty===p?pc(p)+'22':'transparent'};color:${pc(p)}">${p}</button>`).join('')}
    </div>`;

  // Sort district entries
  const sortEntry=([name,d])=>{
    if(simDetailSortCol==='name')return name;
    if(simDetailSortCol==='seats')return d.s;
    if(simDetailSortCol==='to')return (TO_MAP[name]||{}).to21||0;
    if(focusParties.includes(simDetailSortCol)){
      const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
      return (d.v[simDetailSortCol]||0)/tot;
    }
    if(simDetailSortCol==='status'){
      const st=getSimDistStatus(name,d,simDetailParty);
      return {safe:0,lean:1,battleground:2,tossup:3,absent:4}[st]||5;
    }
    return name;
  };
  const distEntries=Object.entries(selRegion?Object.fromEntries(Object.entries(D).filter(([,d])=>d.r===selRegion)):D)
    .sort((a,b)=>{
      const va=sortEntry(a), vb=sortEntry(b);
      return typeof va==='string'?va.localeCompare(vb)*simDetailSortDir:(va-vb)*simDetailSortDir;
    });

  const sdTh=(col,label)=>`<div style="font-size:10px;font-weight:700;color:${focusParties.includes(col)?pc(col):'var(--text3)'};text-transform:uppercase;letter-spacing:.07em;cursor:pointer;user-select:none" onclick="simDetailSortCol='${col}';simDetailSortDir=simDetailSortCol==='${col}'?-simDetailSortDir:1;renderSimulationDetails()">${label}${simDetailSortCol===col?(simDetailSortDir>0?' ▲':' ▼'):' ↕'}</div>`;
  const rows=distEntries.map(([name,d])=>{
    const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
    const allWinners=rankBySeats(d.v,d.s);
    const to=((TO_MAP[name]||{}).to21||0.45)*100;
    const simD=simResults&&simResults[name];
    const simWinner=simD?(simD.seats?.[0]||d.w21):d.w21;
    const status=getSimDistStatus(name,d,simDetailParty);
    const focusCols=focusParties.map(p=>{
      const v=d.v[p]||0;
      const pct=(v/tot*100).toFixed(1);
      const simSeats=simD&&simD.meanSeats?Math.round(simD.meanSeats[p]||0):allWinners.filter(w=>w===p).length;
      const winProb=simD&&simD.meanSeats?Math.min(1,simD.meanSeats[p]||0):(allWinners.includes(p)?0.9:0);
      const projVotes=simD&&simD.adj?Math.round(simD.adj[p]||0):v;
      return `<div style="text-align:center;padding:0 2px">
        <div style="font-size:11px;font-weight:700;color:${pc(p)}">${pct}%</div>
        <div style="font-size:10px;color:${simSeats>0?pc(p):'var(--text4)'}">${simSeats>0?simSeats+'s':'—'}</div>
        <div style="font-size:9px;color:var(--text3)">${projVotes.toLocaleString()}</div>
        <div style="font-size:9px;color:var(--text3)">${Math.round(winProb*100)}%</div>
      </div>`;
    }).join('');
    const otherWinners=[...new Set((simD?simD.seats:allWinners).filter(p=>!focusParties.includes(p)))];
    const otherChips=otherWinners.map(p=>`<span onclick="event.stopPropagation();simDetailParty='${p}';renderSimulationDetails()" style="display:inline-flex;align-items:center;padding:2px 7px;border-radius:4px;font-size:10px;font-weight:700;cursor:pointer;background:${pc(p)}18;color:${pc(p)};border:1px solid ${pc(p)}30;margin:1px">${p}</span>`).join('');
    return `<div onclick="openSimDistModal('${name.replace(/'/g,"\\'")}')" style="display:grid;grid-template-columns:180px 40px 32px 100px 100px 100px 1fr 80px;align-items:center;padding:7px 10px;border-bottom:1px solid var(--border);cursor:pointer;transition:background .1s" onmouseenter="this.style.background='var(--surface2)'" onmouseleave="this.style.background='transparent'">
      <div style="min-width:0">
        <div style="font-size:12px;font-weight:700;color:var(--navy);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${name}</div>
        <div style="font-size:10px;color:var(--text3)">${d.r.replace('Tanger - Tétouan - Al Hoceima','TTH').replace('Casablanca - Settat','Casa').replace('Rabat - Salé - Kénitra','R-S-K').replace(' - Khénifra','').slice(0,24)}</div>
      </div>
      <div style="text-align:center;font-size:11px;font-weight:700;color:var(--text3)">${d.s}</div>
      <div style="text-align:center;font-size:10px;color:var(--text3)">${to.toFixed(0)}%</div>
      ${focusCols}
      <div style="display:flex;gap:2px;flex-wrap:wrap">${otherChips}</div>
      <div style="text-align:right"><span class="pill ${simDistPillClass(status)}">${simDistLabel(status)}</span></div>
    </div>`;
  }).join('');
  ...
  <div class="card" style="margin-bottom:12px;padding:14px 16px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
      <div class="card-title" style="margin:0">${Object.keys(distEntries.reduce((a,[,d])=>(a[d.r]=1,a),{})).length > 1 ? `${distEntries.length} Circonscriptions` : selRegion} — ${isCustomSim?'Votre Simulation':'Prévision Baseline (10k runs)'}</div>
      <div style="font-size:11px;color:var(--text3)">%/sièges/voix/win% · Cliquer pour explorer</div>
    </div>
    ${partySel}
  </div>
  <div class="card" style="padding:0;overflow:hidden">
    <div style="display:grid;grid-template-columns:180px 40px 32px 100px 100px 100px 1fr 80px;align-items:center;padding:8px 10px;background:var(--surface2);border-bottom:2px solid var(--border);position:sticky;top:0;z-index:2">
      ${sdTh('name','Circonscription')}
      ${sdTh('seats','S')}
      ${sdTh('to','TO')}
      ${focusParties.map(p=>sdTh(p,p)).join('')}
      <div style="font-size:10px;font-weight:700;color:var(--text3)">Autres</div>
      ${sdTh('status','Statut')}
    </div>
    ${rows}
  </div>
  <div id="sim-dist-modal" onclick="if(event.target===this)closeSimDistModal()" style="display:none;position:fixed;inset:0;z-index:9000;background:rgba(0,0,0,.45);align-items:center;justify-content:center">
    <div id="sim-dist-modal-body" style="background:var(--surface);border-radius:12px;width:600px;max-height:88vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.3)"></div>
  </div>`;
  document.getElementById('view-content').innerHTML=html;
  renderRightPanel();
}

function openSimDistModal(distName){
  const d=D[distName];if(!d)return;
  const modal=document.getElementById('sim-dist-modal');if(!modal)return;
  const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
  const sorted=Object.entries(d.v).sort((a,b)=>b[1]-a[1]);
  const allWinners2021=rankBySeats(d.v,d.s);
  const uniqueWinners2021=[...new Set(allWinners2021)];
  const simD=simResults&&simResults[distName];
  const simSeatsArr=simD?simD.seats||[]:allWinners2021;
  const simUniqueWinners=[...new Set(simSeatsArr)];
  const simWinner=simSeatsArr[0]||d.w21;
  const winner=uniqueWinners2021[0];
  const closeLosers=sorted.slice(d.s,d.s+2).map(([p])=>p).filter(Boolean);
  const allRanked=sorted.map(([p,v],i)=>({p,v,pct:v/tot*100,rank:i+1}));
  const adv=buildDistAdvantage(distName,d,winner,closeLosers);
  const to=TO_MAP[distName]||{};
  const to21=((to.to21||0.45)*100).toFixed(1);
  const fp=simDetailParty;
  const fpVotes=d.v[fp]||0;
  const fpPct=(fpVotes/tot*100).toFixed(1);
  const fpRank=sorted.findIndex(([p])=>p===fp)+1;
  const fpWins=allWinners2021.filter(w=>w===fp).length;
  const fpChance=simD&&simD.meanSeats?Math.min(1,simD.meanSeats[fp]||0):fpWins?0.9:0;
  const fpStatus=getSimDistStatus(distName,d,fp);

  const bodyHtml=`
  <div style="padding:20px 24px">
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px">
      <div>
        <div style="font-size:18px;font-weight:800;color:var(--navy)">${distName}</div>
        <div style="font-size:11px;color:var(--text3);margin-top:2px">${d.r} · ${d.s} seats · ${d.u?'Urban':'Rural'} · TO ${to21}%</div>
      </div>
      <button onclick="closeSimDistModal()" style="font-size:20px;color:var(--text3);background:none;border:none;cursor:pointer;padding:0 0 0 12px;line-height:1">×</button>
    </div>
    <div style="background:${pc(winner)}12;border:2px solid ${pc(winner)}40;border-radius:8px;padding:12px 16px;margin-bottom:12px">
      <div style="display:flex;align-items:center;justify-content:space-between">
        <div>
          <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:3px">2021 Rank-1</div>
          <div style="font-size:22px;font-weight:900;color:${pc(winner)}">${winner}</div>
          <div style="font-size:12px;color:var(--text2);margin-top:2px">${((d.v[winner]||0)/tot*100).toFixed(1)}% · ${(d.v[winner]||0).toLocaleString()} votes</div>
        </div>
        <div style="text-align:right">
          <div style="font-size:10px;color:var(--text3);margin-bottom:2px">${simD&&!simResults._baseline?'Sim':'Forecast'} 2026</div>
          <div style="font-size:20px;font-weight:800;color:${pc(simWinner)}">${simWinner}</div>
        </div>
      </div>
    </div>
    <div style="background:${pc(fp)}10;border:2px solid ${pc(fp)}40;border-radius:8px;padding:12px 16px;margin-bottom:12px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:${pc(fp)};margin-bottom:6px">${fp} — selected party</div>
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        <div style="text-align:center"><div style="font-size:18px;font-weight:800;color:${pc(fp)}">${fpPct}%</div><div style="font-size:10px;color:var(--text3)">Vote share</div></div>
        <div style="text-align:center"><div style="font-size:18px;font-weight:800;color:var(--navy)">#${fpRank}</div><div style="font-size:10px;color:var(--text3)">Rank</div></div>
        <div style="text-align:center"><div style="font-size:18px;font-weight:800;color:${fpWins>0?'var(--green)':'var(--red)'}">${fpWins}s</div><div style="font-size:10px;color:var(--text3)">2021 seats</div></div>
        <div style="text-align:center"><div style="font-size:18px;font-weight:800;color:${fpChance>=0.6?'var(--green)':fpChance>=0.35?'var(--amber)':'var(--red)'}">${(fpChance*100).toFixed(0)}%</div><div style="font-size:10px;color:var(--text3)">Win prob</div></div>
      </div>
      <div style="margin-top:8px;padding-top:8px;border-top:1px solid ${pc(fp)}30">
        <span class="pill ${simDistPillClass(fpStatus)}">${simDistLabel(fpStatus)}</span>
        ${fpWins===0&&fpRank<=d.s+1?`<span style="font-size:11px;color:var(--amber);margin-left:8px">First loser — within striking distance</span>`:''}
        ${fpWins>0&&fpChance<0.5?`<span style="font-size:11px;color:var(--red);margin-left:8px">Seat at risk in simulation</span>`:''}
        ${fpWins>0&&fpChance>=0.85?`<span style="font-size:11px;color:var(--green);margin-left:8px">Seat appears secure</span>`:''}
      </div>
    </div>
    <div style="margin-bottom:12px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:7px">Seat allocation (${simD&&!simResults._baseline?'simulation':'2021 baseline'})</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        ${(simD&&!simResults._baseline?simUniqueWinners:uniqueWinners2021).map(p=>{
          const s=(simD&&!simResults._baseline?simSeatsArr:allWinners2021).filter(w=>w===p).length;
          return `<div style="padding:8px 13px;border-radius:7px;background:${pc(p)}15;border:2px solid ${p===fp?pc(p):pc(p)+'40'}">
            <div style="font-size:13px;font-weight:900;color:${pc(p)}">${p}</div>
            <div style="font-size:12px;font-weight:700;color:var(--navy)">${s}s</div>
            <div style="font-size:10px;color:var(--text3)">${((d.v[p]||0)/tot*100).toFixed(1)}%</div>
          </div>`;
        }).join('')}
      </div>
    </div>
    <div style="margin-bottom:12px">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:7px">Full vote ranking</div>
      ${allRanked.slice(0,Math.min(allRanked.length,9)).map((r,i)=>{
        const isSeat=i<d.s;
        const isCutoff=i===d.s;
        const barW=(r.v/sorted[0][1]*100).toFixed(0);
        const isFocus=r.p===fp;
        return `${isCutoff?`<div style="display:flex;align-items:center;gap:6px;margin:4px 0;font-size:10px;color:var(--red);font-weight:700"><div style="flex:1;height:1px;background:var(--red)"></div>CUTOFF<div style="flex:1;height:1px;background:var(--red)"></div></div>`:''}
        <div style="display:flex;align-items:center;gap:8px;padding:4px ${isFocus?'4':'0'}px;${isSeat?'':'opacity:.7'}${isFocus?';background:'+pc(fp)+'0a;border-radius:4px':''};border-bottom:1px solid var(--border)">
          <span style="font-size:10px;color:var(--text3);min-width:14px">${r.rank}</span>
          <div style="width:8px;height:8px;border-radius:2px;background:${pc(r.p)};flex-shrink:0"></div>
          <span style="font-size:12px;font-weight:${isSeat||isFocus?'700':'400'};color:${pc(r.p)};min-width:36px">${r.p}${isFocus?' ◀':''}</span>
          <div style="flex:1;height:5px;background:var(--border);border-radius:3px"><div style="height:100%;width:${barW}%;background:${pc(r.p)};border-radius:3px"></div></div>
          <span style="font-size:11px;min-width:36px;text-align:right">${r.pct.toFixed(1)}%</span>
          <span style="font-size:10px;color:var(--text3);min-width:58px;text-align:right">${r.v.toLocaleString()}</span>
          ${isSeat?`<span style="font-size:10px;color:var(--green);font-weight:700;min-width:40px">✓ SEAT</span>`:`<span style="min-width:40px"></span>`}
        </div>`;
      }).join('')}
    </div>
    <div style="background:var(--navy);color:#fff;border-radius:8px;padding:16px;margin-bottom:12px">
      <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#a8d4e8;margin-bottom:10px">Why ${winner} won — and how</div>
      <div style="display:flex;flex-direction:column;gap:7px;font-size:12px;line-height:1.65">
        <div><span style="color:${pc(winner)};font-weight:700">◆ Vote lead: </span>${adv.leadText}</div>
        <div><span style="color:#a8d4e8;font-weight:700">◆ History: </span>${adv.histText}</div>
        <div><span style="color:#a8d4e8;font-weight:700">◆ Territory: </span>${adv.geoText}</div>
        ${adv.commText?`<div><span style="color:#a8d4e8;font-weight:700">◆ Communes: </span>${adv.commText}</div>`:''}
        <div><span style="color:#a8d4e8;font-weight:700">◆ Turnout: </span>${adv.toText}</div>
      </div>
    </div>
    <div>
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:7px">Winner trajectory 2011 → 2026</div>
      <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
        ${['11','15','16','21'].map(yr=>{const w=d['w'+yr];return w?`<div style="text-align:center"><div style="font-size:9px;color:var(--text3);margin-bottom:3px">20${yr}</div><div style="width:36px;height:36px;border-radius:6px;background:${pc(w)}18;border:1px solid ${pc(w)}40;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;color:${pc(w)}">${w}</div></div>`:'';}).filter(Boolean).join('<span style="color:var(--border2);font-size:16px;margin-top:10px">→</span>')}
        <span style="color:var(--border2);font-size:16px;margin-top:10px">→</span>
        <div style="text-align:center">
          <div style="font-size:9px;color:var(--teal);margin-bottom:3px;font-weight:700">2026↗</div>
          <div style="width:36px;height:36px;border-radius:6px;background:${pc(simWinner)}22;border:2px solid ${pc(simWinner)};display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;color:${pc(simWinner)}">${simWinner}</div>
        </div>
      </div>
    </div>
  </div>`;

  document.getElementById('sim-dist-modal-body').innerHTML=bodyHtml;
  modal.style.display='flex';
}

function closeSimDistModal(){
  const m=document.getElementById('sim-dist-modal');
...
function renderJSAPview(){
  ...
  // For each district, compute at jsapForce what seat JSAP would take and from whom
  const computeJSAPSeatImpact=(name,d,force)=>{
    if(!force||force===0)return null;
    const to=TO_MAP[name]||{};
    const voixAdd=to.voixAdd||0;
    const jsapScore=getJSAPscore(d,name);
    const jsapVotes=voixAdd*force/100*jsapScore;
    if(!jsapVotes)return null;
    const tot=Object.values(d.v).reduce((a,b)=>a+b,0)||1;
    // Sort parties by votes
    const ranked=Object.entries(d.v).sort((a,b)=>b[1]-a[1]);
    // Add JSAP and re-rank
    const withJSAP=[...ranked,['JSAP',jsapVotes]].sort((a,b)=>b[1]-a[1]);
    // Current winners without JSAP (Hare/largest remainder approximation)
    const baseWinners=ranked.slice(0,d.s).map(([p])=>p);
    // Winners with JSAP
    const newWinners=withJSAP.slice(0,d.s).map(([p])=>p);
    // Who loses a seat?
    const baseCounts={}; baseWinners.forEach(p=>baseCounts[p]=(baseCounts[p]||0)+1);
    const newCounts={}; newWinners.forEach(p=>newCounts[p]=(newCounts[p]||0)+1);
    const loser=Object.entries(baseCounts).find(([p,n])=>(newCounts[p]||0)<n)?.[0];
    const jsapWins=newWinners.includes('JSAP');
    const lastSeatP=ranked[d.s-1]?.[0];
    const firstLoserP=ranked[d.s]?.[0];
    const gap=ranked[d.s-1]&&ranked[d.s]?((ranked[d.s-1][1]-ranked[d.s][1])/tot*100).toFixed(1):'?';
    return {jsapVotes:Math.round(jsapVotes),jsapWins,loser,lastSeatP,firstLoserP,gap,baseWinners,newWinners};
  };

  const html=`
  <div class="sap-card">
    <div class="sap-title">⚡ JSAP — Jeunes Sans Appartenance Politique</div>
    <div class="sap-desc">Le modèle JSAP représente des campagnes indépendantes portées par des jeunes qui mobilisent les non-votants urbains. Il cible les circonscriptions à faible participation législative, forte concentration jeune, et faible loyauté partisane — captant l'électorat non-votant qu'aucun parti traditionnel n'atteint.</div>
    ...
    <div style="font-size:11px;color:var(--text2)">% de non-votants mobilisables captés par le mouvement JSAP</div>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
    <div class="card">
      <div class="card-title">Top districts fertiles JSAP</div>
      ${sapArr.slice(0,15).map((d,i)=>{
        ...
        const impact=jsapForce>0?computeJSAPSeatImpact(d.n,d,jsapForce):null;
        return `<div style="border-bottom:1px solid var(--border);padding:8px 0" id="jsap-row-${i}">
          <div style="display:flex;align-items:center;justify-content:space-between;cursor:pointer" onclick="toggleJSAPDetail('jsap-detail-${i}')">
            <div>
              ...
              <div style="font-size:10px;color:var(--text3);margin-top:1px">Jeunes ${d.yth||14}% · TO ${(d.to21*100).toFixed(0)}% · ${d.u?'Urbain':'Rural'}</div>
            </div>
            <div style="display:flex;gap:6px;align-items:center">
              <div class="pill" style="background:#f0fdfa;color:#0891b2">${(d.sap*100).toFixed(0)}%</div>
              ${impact&&impact.jsapWins&&impact.loser?`<div class="pill" style="background:#fee2e2;color:#dc2626;font-size:9px">−${impact.loser}</div>`:''}
              <span style="font-size:10px;color:var(--text3)">▼</span>
            </div>
          </div>
          <div id="jsap-detail-${i}" style="display:none;margin-top:8px;padding:10px 12px;background:#f0fdfa;border-radius:6px;font-size:11px">
            <div style="font-weight:700;color:#0891b2;margin-bottom:8px">Pourquoi fertile pour JSAP</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:8px">
              <div>🗳️ TO 2021: <strong>${(d.to21*100).toFixed(0)}%</strong> ${d.to21<0.35?'⚡ Très bas':d.to21<0.45?'↓ Bas':''}</div>
              <div>👥 Jeunes: <strong>${d.yth||14}%</strong> ${(d.yth||14)>18?'⚡ Élevé':''}</div>
              <div>🏙️ Type: <strong>${d.u?'Urbain':'Rural'}</strong></div>
              <div>💪 Mobilisable: <strong>~${voixAdd.toLocaleString()}</strong> voix</div>
              <div>📊 Écart dernier siège: <strong>${lastGap}pp</strong></div>
              <div>🎯 JSAP à ${jsapForce}%: <strong>+${jsapVotes.toLocaleString()} voix</strong></div>
            </div>
            ${impact?`
            <div style="background:#fff;border-radius:5px;padding:8px;border:1px solid #99f6e4;margin-top:4px">
              <div style="font-size:10px;font-weight:700;color:#0891b2;margin-bottom:4px">IMPACT PROJETÉ À ${jsapForce}%</div>
              ${impact.jsapWins?`
                <div style="font-size:11px;color:#059669;font-weight:600;margin-bottom:4px">✓ JSAP obtient un siège</div>
                ${impact.loser?`<div style="font-size:11px;color:#dc2626"><strong>${impact.loser}</strong> perd son siège au profit de JSAP</div>
                <div style="font-size:10px;color:var(--text3);margin-top:3px">Base: ${impact.baseWinners.join(', ')} → Avec JSAP: ${impact.newWinners.join(', ')}</div>`:
                '<div style="font-size:11px;color:var(--text3)">Repartition des sièges inchangée malgré la présence JSAP</div>'}
              `:
              `<div style="font-size:11px;color:var(--amber)">JSAP (${impact.jsapVotes.toLocaleString()} voix) trop faible pour décrocher un siège<br>
              Écart vs ${impact.lastSeatP}: ${lastGap}pp — il faut plus de force</div>`}
            </div>`:''}
            <div style="margin-top:6px"><a href="#" onclick="event.preventDefault();selectDist('${d.n.replace(/'/g,"\\'")}')" style="color:#0891b2;font-weight:600">Voir profil complet →</a></div>
          </div>
        </div>`;}).join('')}
    </div>
    <div class="card">
      <div class="card-title">Modèle JSAP — paramètres</div>
      <p style="font-size:12px;color:var(--text2);line-height:1.7;margin-bottom:12px">
        Le modèle évalue chaque circonscription sur trois dimensions: <strong>faible participation législative</strong> (districts urbains TO &lt;35%), <strong>concentration jeune</strong> (20–24 ans &gt;18% des inscrits), et <strong>densité urbaine</strong> (mobilisation réseau rapide).
      </p>
      <div class="score-bar"><span class="score-label">Poids participation</span><div class="score-track"><div class="score-fill" style="width:40%;background:var(--teal)"></div></div><span class="score-val">40%</span></div>
      <div class="score-bar"><span class="score-label">Poids jeunesse</span><div class="score-track"><div class="score-fill" style="width:35%;background:#0891b2"></div></div><span class="score-val">35%</span></div>
      <div class="score-bar"><span class="score-label">Poids urbain</span><div class="score-track"><div class="score-fill" style="width:25%;background:#0891b2"></div></div><span class="score-val">25%</span></div>
      <div class="divider"></div>
      ${jsapForce>0?(()=>{
        const impacts=sapArr.slice(0,20).map(d=>{
          const imp=computeJSAPSeatImpact(d.n,d,jsapForce);
          return imp&&imp.jsapWins?{name:d.n,loser:imp.loser,votes:imp.jsapVotes}:null;
        }).filter(Boolean);
        return impacts.length?`
        <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Sièges pris à qui (top 20 districts)</div>
        ${impacts.map(imp=>`<div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid var(--border);font-size:11px">
          <div style="flex:1;color:var(--navy);font-weight:500">${imp.name}</div>
          ${imp.loser?`<span style="color:#dc2626;font-weight:700">−${imp.loser}</span>
          <span style="color:#0891b2;font-weight:700">+JSAP</span>`:'<span style="color:var(--text3)">redistribution interne</span>'}
        </div>`).join('')}
        <div style="font-size:11px;color:var(--teal);font-weight:700;margin-top:8px">Total JSAP: ~${impacts.length} siège(s) possible(s)</div>`:
        `<div style="font-size:11px;color:var(--text3)">À ${jsapForce}% de force, JSAP ne prend aucun siège dans le top 20 — augmentez la force.</div>`;
      })():`<div style="font-size:11px;color:var(--text2)">Bougez le curseur de force pour voir l'impact sur les sièges.</div>`}
      ${jsapForce>0?`<div class="alert al-teal" style="margin-top:8px"><span class="al-icon"></span><span>JSAP à ${jsapForce}% actif — districts urbains avec jeunes &gt;20% verront une perturbation significative.</span></div>`:''}
    </div>
  </div>`;
  document.getElementById('view-content').innerHTML=html;
}
...
function runSim(){
  ...
  if(runBtn){runBtn.textContent='Running…';runBtn.disabled=true;runBtn.style.background='var(--teal)';}
  ...
  setTimeout(()=>{
    for(let run=0;run<mcRuns;run++){
      ...
      for(const [name,d] of Object.entries(D)){
        ...
        // Turnout multiplier: ratio of projected TO to baseline TO
        const baseTo=(TO_MAP[name]||{}).to_leg||(TO_MAP[name]||{}).to21||d.to||0.45;
        const effTo=getEffTO(d,name);
        const toMult=baseTo>0?effTo/baseTo:1.0;
        // District-specific PJD multiplier (per RNI note: Casa +200%, Kénitra +30%, other urban +300%)
        const isCasa=d.r==='Casablanca - Settat'&&d.u===1;
        const isKenitra=name.includes('Kénitra')||(d.r==='Rabat - Salé - Kénitra'&&name==='Kénitra');
        const pjdDistMult=isCasa?3.0:isKenitra?1.3:(d.u===1?4.0:1.0);
        const adjV={};
        for(const [p,v] of Object.entries(d.v)){
          ...
          // Apply district-specific PJD multiplier for baseline-like behaviour
          const pjdBoost=(p==='PJD'&&pjdDistMult>1)?pjdDistMult:1.0;
          // Apply turnout multiplier to scale total votes up/down proportionally
          adjV[p]=Math.max(0,(v+globalSw+distSw+noise+iBonus)*orgaMult*toMult*pjdBoost);
        }
        if(jsapForce>0){
          const jsapScore=getJSAPscore(d,name);
          const to=(TO_MAP[name]||{});
        ...
        // Apply guaranteed seats first (political input — locked regardless of vote rank)
        ...
        for(const [gp,gn] of gEntries){
          ...
          guaranteedCount[gp]=give;
          seatsLeft-=give;
        }
        // Fill remaining seats by vote rank — guaranteed parties can still win MORE seats
    ...
    renderRightPanel();
    setTopTab(activeTopTab||'forecast');
    buildSidebar();
    if(runBtn){
...
function resetSim(){
  ...
  if(rb){rb.innerHTML=' Run Simulation';rb.style.background='';rb.disabled=false;}
}

//  RIGHT PANEL NATIONAL 
function renderRightPanel(){
  const localSeats=getSeats();
  const regSeats=getLRNationalSeats();
  const combined=getCombinedSeats();
  const sorted=Object.entries(combined).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
  const total=TOTAL();
  document.getElementById('rp-head').textContent='Projection Nationale';
  const html=`
  <div class="sec-hd">Sièges combinés (locales + régionales)</div>
  <div class="seat-bar">${sorted.map(([p,n])=>`<div style="width:${(n/total*100).toFixed(1)}%;background:${pc(p)}" title="${p}: ${n}"></div>`).join('')}</div>
  ${sorted.map(([p,n])=>{
    const loc=localSeats[p]||0; const reg=regSeats[p]||0;
    const b=HIST_NAT['2021'][p]||0; const delta=simResults?loc-b:null;
    const ci=simResults&&simResults._ci?simResults._ci[p]:'';
    // Projected votes from sim
    let projVotes=0;
    if(simResults&&!simResults._baseline){
      Object.entries(D).forEach(([name,d])=>{
        const simD=simResults[name];
        if(simD&&simD.adj)projVotes+=(simD.adj[p]||0);
        else projVotes+=(d.v[p]||0);
      });
    } else {
      Object.values(D).forEach(d=>{projVotes+=(d.v[p]||0);});
    }
    return `<div class="party-row">
      <div class="p-swatch" style="background:${pc(p)}"></div>
      <span class="p-name" style="color:${pc(p)}">${p}</span>
      ...
      <div style="text-align:right">
        <span class="p-seats">${n}</span>
        <div style="font-size:9px;color:var(--text3)">${loc}L+${reg}R</div>
      </div>
      ${delta!==null?`<span class="p-delta" style="color:${delta>0?'var(--green)':delta<0?'var(--red)':'var(--text3)'}">${delta>=0?'+':''}${delta}</span>`:''}
      ${ci?`<span class="p-ci">${ci}</span>`:''}
    </div>
    ${simResults&&!simResults._baseline?`<div style="font-size:9px;color:var(--text3);padding:0 2px 3px;text-align:right">${Math.round(projVotes).toLocaleString()} voix proj.</div>`:''}`;
  }).join('')}
  ...
  <div id="scen-dist-pick" style="display:none;margin-bottom:8px">
    <select id="scen-dist-sel" onchange="scenDistName=this.value;renderRightPanel()" style="font-size:11px;padding:5px 8px">
      <option value="">Choose district...</option>
      ${Object.keys(D).sort().map(n=>`<option value="${n}"${scenDistName===n?' selected':''}>${n}</option>`).join('')}
    </select>
  </div>
...
function runScenSet(id){
  ...
  for(const [name,d] of Object.entries(D)){
    ...
    let seatsLeft=d.s;
    const alloc=[];
    for(const [gp,gn] of Object.entries(gSeats).filter(([,n])=>n>0)){
      const give=Math.min(gn,seatsLeft);
      for(let i=0;i<give;i++){alloc.push(gp);}
      seatsLeft-=give;
    }
    for(const [p] of ranked){if(seatsLeft<=0)break;alloc.push(p);seatsLeft--;}
...
function renderPartyProfileView(party){
  ...
  <div class="pp-header">
    <div style="display:flex;align-items:flex-start;justify-content:space-between">
      <div style="flex:1">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
          ...
          ${toGain?`<span class="pill pill-red">${toGain} at risk of loss</span>`:''}
        </div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;font-size:11px">
      ...
      <div style="background:#fdf4ff;border-radius:5px;padding:7px 10px;text-align:center"><div style="font-weight:700;color:var(--purple);font-size:14px">${battleground.length}</div><div style="color:var(--text3)">Prob. Loss</div></div>
    </div>
  </div>

  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
    ${(()=>{
      // Districts won in 2021 but sim shows high loss probability
      const simLoss=[...dominated,...strongholds,...competitive].filter(o=>o.seatChance<0.5).sort((a,b)=>a.seatChance-b.seatChance);
      const cats=[
        ...
        {label:' High Probability of Loss',data:simLoss,color:'var(--red)',bg:'var(--red-bg)',desc:'Won in 2021 — simulation projects losing in 2026'},
    ...
    <div class="card" style="border-left:3px solid ${cat.color}">
      ...
      ${cat.data.length?cat.data.map(o=>renderPartyDistRow(o,party,cat.color)).join(''):'<div style="font-size:11px;color:var(--text3)">None</div>'}
    </div>`).join('');
    })()}
  </div>
...
function buildCommsSystemPrompt(){
  ...
  const partyCtx=MAJOR.map(p=>{
    const distWon=Object.entries(D).filter(([n,d])=>rankBySeats(d.v,d.s).includes(p)).length;
  ...
  try{
    const response=await fetch('/api/claude',{
      ...
      headers:{
        ...
        'Authorization':'Bearer '+sessionStorage.getItem('intelx_s')
      },
      body:JSON.stringify({
...
function renderNewsView(){
  document.getElementById('view-tabs').innerHTML=`
    <button class="vtab" onclick="setCommsSubTab('chat')">AI Strategy Chat</button>
    <button class="vtab on" onclick="setCommsSubTab('news')">📰 News Monitor</button>
  `;
...
async function exportPDF(){
  ...
  try{
    ...
    const payload = {
      national: seats,
      ci: BASELINE_SIM._ci||{},
      ci_lo: BASELINE_SIM._ci_lo||{},
      ci_hi: BASELINE_SIM._ci_hi||{},
      runs: BASELINE_SIM._runs||10000,
      label: BASELINE_SIM._label||'2026 Forecast',
      districts: Object.entries(D).map(([name,d])=>{
        const sim = simResults&&simResults[name];
        return {
          name, seats:d.s, w21:d.w21, region:d.r,
          projected: sim ? sim.seats||[] : [],
          meanSeats: sim ? sim.meanSeats||{} : {},
        };
      })
    };
    const res = await fetch('/api/report', {
      method:'POST',
      headers:{'Content-Type':'application/json','Authorization':'Bearer '+sessionStorage.getItem('intelx_s')},
    ...
    a.href=url; a.download='intelX_Simulation_Report_2026.pdf';
