...
function setTopTab(tab){
  ...
  if(!selDist||tab==='dashboard'||tab==='battleground'||tab==='intel'||tab==='simdetails'||tab==='lr'||tab==='jsap'||tab==='party'||tab==='scensets'||tab==='h2h'||tab==='strategic'||tab==='comms'||tab==='news'||tab==='orga'){
...
function renderDashboard(){
  ...
  <div class="card">
    <div class="card-title" style="display:flex;justify-content:space-between">
      <span>Listes régionales — aperçu (${Object.values(LR).reduce((a,v)=>a+v.sieges,0)} sièges)</span>
      <span style="font-size:10px;color:var(--teal);cursor:pointer" onclick="setTopTab('lr')">Détail complet →</span>
    </div>
    <div style="display:grid;grid-template-columns:repeat(${Object.keys(LR).length},1fr);gap:4px">
      ${Object.entries(LR).map(([lrName,{sieges,region}])=>{
        const lr=calcRegionalSeats();
        const seats=lr[lrName]?.seats||{};
        const winner=Object.entries(seats).sort((a,b)=>b[1]-a[1])[0];
        const isScoped=!selRegion||selRegion===region;
        return `<div style="text-align:center;opacity:${isScoped?1:.3};cursor:pointer" onclick="setTopTab('lr')">
          <div style="font-size:9px;color:var(--text3);margin-bottom:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${lrName.split('-')[0].trim().slice(0,8)}</div>
          <div style="background:${winner?pc(winner[0])+'22':'var(--surface2)'};border:1px solid ${winner?pc(winner[0])+'44':'var(--border)'};border-radius:4px;padding:4px 2px">
            <div style="font-size:11px;font-weight:800;color:${winner?pc(winner[0]):'var(--text4)'}">${winner?winner[0]:'—'}</div>
            <div style="font-size:10px;color:var(--text3)">${sieges}s</div>
          </div>
        </div>`;
      }).join('')}
    </div>
    ...
    for(let run=0;run<mcRuns;run++){
      ...
      for(const [name,d] of Object.entries(D)){
        ...
        for(const [p,v] of Object.entries(d.v)){
          ...
          adjV[p]=Math.max(0,(v+globalSw+distSw+noise+iBonus)*orgaMult*toMult);
        }
        if(jsapForce>0){
          const jsapScore=getJSAPscore(d,name);
          const to=(TO_MAP[name]||{});
